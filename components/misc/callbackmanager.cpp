...
namespace Misc
{
    ...
    void CallbackManager::installRenderStages()
    {
        ...
        if (renderer != nullptr)
        {
            ...
            Log(Debug::Info) << "Android VR callback manager installing render stages";
#endif
...
#if defined(ANDROID)
        static unsigned int sAndroidCallbackLogs = 0;
        if (sAndroidCallbackLogs < 80)
        {
            ++sAndroidCallbackLogs;
            Log(Debug::Warning) << "Android VR callback manager dispatch"
...
#if defined(ANDROID)
        Log(Debug::Warning) << "Android VR callback manager add"
                            << " stage=" << static_cast<int>(stage)
                            << " frame=" << callbackInfo.frame
                            << " callbacks=" << mUserCallbacks[static_cast<int>(stage)].size();
#endif
    }
...
#if defined(ANDROID)
        static unsigned int sAndroidNullFboLogs = 0;
        if (!getFrameBufferObject())
        {
            if (sAndroidNullFboLogs < 120)
            {
                ++sAndroidNullFboLogs;
                Log(Debug::Warning) << "Android VR renderstage diag: draw with null FBO"
                                    << " view=" << static_cast<int>(mView)
                                    << " callbackManager=" << (mCallbackManager ? "yes" : "no")
                                    << " camera=" << (_camera.valid() ? _camera->getName() : std::string("<none>"))
                                    << " cameraTarget="
                                    << (_camera.valid() ? static_cast<int>(_camera->getRenderTargetImplementation()) : -1)
                                    << " cameraAttachments="
                                    << (_camera.valid() ? static_cast<int>(_camera->getBufferAttachmentMap().size()) : -1);
            }
        }
#endif
