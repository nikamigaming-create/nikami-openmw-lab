...
namespace XR
{
    ...
    {
        handleEvents();
        if (mWaitFrameSuppressedUntilFocus && mState == XR_SESSION_STATE_FOCUSED)
        {
            Log(Debug::Warning) << "OpenXR: resuming frame loop after focused state recovered";
            mWaitFrameSuppressedUntilFocus = false;
        ...
        uint64_t frameNo, bool& shouldRender, uint64_t& predictedDisplayTime, uint64_t& predictedDisplayPeriod)
    {
        ...
        const XrResult waitResult = CHECK_XRCMD(xrWaitFrame(mXrSession, &frameWaitInfo, &frameState));
        if (XR_FAILED(waitResult))
        {
            ...
            mWaitFrameSuppressedUntilFocus = true;
            Log(Debug::Warning) << "OpenXR: dropping frame " << frameNo
                                << " after xrWaitFrame failure; suppressing frame loop until focused state recovers";
            return;
        }
    ...
    void Session::syncFrameRender(VR::Frame& frame)
    {
        ...
        const XrResult beginResult = CHECK_XRCMD(xrBeginFrame(mXrSession, &frameBeginInfo));
        if (XR_FAILED(beginResult))
        {
            frame.shouldRender = false;
            frame.predictedDisplayTime = 0;
            Log(Debug::Warning) << "OpenXR: dropping frame " << frame.frameNumber
                                << " after xrBeginFrame failure";
        }
    }
    ...
    bool Session::handleSessionStateChanged(const XrEventDataSessionStateChanged& stateChangedEvent)
    {
        ...
        switch (mState)
        {
            ...
            {
                mAppShouldSyncFrameLoop = !mWaitFrameSuppressedUntilFocus;
                mAppShouldReadInput = false;
                mXrSessionShouldStop = false;
