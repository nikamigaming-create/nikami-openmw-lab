...
namespace
{
    bool isVrDebugExtensionDisabled(const std::string& extension)
    {
        try
        {
            return Settings::Manager::getBool("disable " + extension, "VR Debug");
        }
        catch (const std::exception& e)
        {
            Log(Debug::Warning) << "Missing VR Debug setting for " << extension << ", leaving extension enabled: "
                                << e.what();
            return false;
        }
    }
...
namespace XR
{
    ...
    bool Platform::selectDirectX()
    {
        ...
        if (isVrDebugExtensionDisabled(XR_KHR_D3D11_ENABLE_EXTENSION_NAME))
        {
    ...
    bool Platform::selectOpenGL()
    {
        ...
        if (isVrDebugExtensionDisabled(XR_KHR_OPENGL_ES_ENABLE_EXTENSION_NAME))
        {
            Log(Debug::Verbose) << "  " << XR_KHR_OPENGL_ES_ENABLE_EXTENSION_NAME << ": disabled (by config)";
            return false;
        }
        ...
        if (isVrDebugExtensionDisabled(XR_KHR_OPENGL_ENABLE_EXTENSION_NAME))
        {
        ...
        XrInstance instance, XrSystemId systemId, [[maybe_unused]] const std::string& systemName)
    {
        ...
        graphicsBindings.display = eglGetCurrentDisplay();
        graphicsBindings.context = eglGetCurrentContext();
        graphicsBindings.config = reinterpret_cast<EGLConfig>(0);

        Log(Debug::Warning) << "Android OpenXR OpenGLES binding display=" << graphicsBindings.display
                            << " context=" << graphicsBindings.context << " config=" << graphicsBindings.config;

        if (graphicsBindings.display == EGL_NO_DISPLAY)
            Log(Debug::Warning) << "Missing EGL display for Android OpenXR session";
        if (graphicsBindings.context == EGL_NO_CONTEXT)
            Log(Debug::Warning) << "Missing EGL context for Android OpenXR session";
...
#if defined(__ANDROID__) && defined(XR_USE_GRAPHICS_API_OPENGL_ES)
            if (std::find(mMWColorFormatsGL.begin(), mMWColorFormatsGL.end(), GL_SRGB8_ALPHA8)
                != mMWColorFormatsGL.end())
                glFormat = GL_SRGB8_ALPHA8;
            else if (std::find(mMWColorFormatsGL.begin(), mMWColorFormatsGL.end(), GL_RGBA8)
                != mMWColorFormatsGL.end())
                glFormat = GL_RGBA8;
            else
#endif
