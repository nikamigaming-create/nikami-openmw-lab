...
namespace XR
{
    ...
    namespace
    {
        bool isExtensionDisabled(const std::string& extension)
        {
            try
            {
    ...
    extern "C" void openmw_initialize_openxr_loader(JavaVM* vm, JNIEnv* env, jobject activity)
    {
        ...
        if (sAndroidActivity)
            env->DeleteGlobalRef(sAndroidActivity);
            ...
            && initializeLoader)
        {
            ...
            loaderInitInfoAndroid.applicationVM = sAndroidVM;
            loaderInitInfoAndroid.applicationContext = sAndroidActivity;
            initializeLoader(reinterpret_cast<const XrLoaderInitInfoBaseHeaderKHR*>(&loaderInitInfoAndroid));
        }
    }

    extern "C" JNIEXPORT void JNICALL Java_org_openmw_vrquest_OpenMWActivity_initializeOpenXRLoader(
        JNIEnv* env, jobject activity)
    {
        JavaVM* vm = nullptr;
        env->GetJavaVM(&vm);
        openmw_initialize_openxr_loader(vm, env, activity);
    }
#endif
...
#ifdef __ANDROID__
        autoExtensions.push_back(XR_KHR_ANDROID_CREATE_INSTANCE_EXTENSION_NAME);
#endif
...
#ifdef __ANDROID__
        requestExtension(XR_EXT_HAND_TRACKING_EXTENSION_NAME);
#endif
    ...
    XrInstance Extensions::createXrInstance(const std::string& name)
    {
        ...
        if (sAndroidVM && sAndroidActivity)
        {
            androidCreateInfo.applicationVM = sAndroidVM;
            androidCreateInfo.applicationActivity = sAndroidActivity;
            androidCreateInfo.next = createInfo.next;
            createInfo.next = &androidCreateInfo;
        }
        else
            Log(Debug::Warning) << "Android OpenXR loader was not initialized before xrCreateInstance";
#endif
