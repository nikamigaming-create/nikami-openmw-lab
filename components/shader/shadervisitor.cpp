    ...
    void ShaderVisitor::createProgram(const ShaderRequirements& reqs)
    {
        ...
        if (daoFaceShader != nullptr && *daoFaceShader != '\0' && std::string_view(daoFaceShader) != "0")
        {
            ...
            if (isFaceSurface)
            {
                ...
                // reconstructed Face1 basis is supplied explicitly.
                ...
                {
                    const std::array<std::pair<const char*, const char*>, 10> materialTextures = { {
                        ...
                        { "daoTattooMask", "uh_tat_p01.dds" },
                        { "daoBeckmannLut", "beckmann.dds" },
                    } };
                    unsigned int textureUnit = static_cast<unsigned int>(textureAttributes.size());
                    const bool useLegacyFaceUv = [] {
                        const char* value = std::getenv("OPENMW_DAO_FACE_LEGACY_UV");
                        return value != nullptr && *value != '\0' && std::string_view(value) != "0";
                    }();
                    for (const auto& [semantic, fileName] : materialTextures)
                    {
                        if (std::string_view(semantic) != "daoBeckmannLut" && !useLegacyFaceUv)
                            continue;
