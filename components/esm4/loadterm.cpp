...
namespace
{
    ...
    {
        if (reader.subRecordHeader().dataSize == sEmptySubRecordSize)
    ...
    ESM4::TargetCondition readCondition(ESM4::Reader& reader)
    {
        ...
        // These two functions carry FormIDs in every occurrence in the frozen
        // FNV TERM corpus, even though the shared structure stores them as raw
        // integers.
    ...
    bool canCloseMenu(const ESM4::Terminal::MenuItem& item, MenuPhase phase)
    {
        if (phase == MenuPhase::AfterHeader)
            return item.mScript.scriptHeader.compiledSize == sEmptySubRecordSize;
        ...
        while (reader.getSubRecordHeader())
        {
            ...
            switch (subHdr.typeId)
            {
                ...
                case ESM::fourCC("SNAM"):
                    if (subHdr.dataSize == sFormIdSerializedSize)
    ...
    void readZString(ESM4::Reader& reader, ESM::Path& value, std::string_view field)
    {
        if (reader.subRecordHeader().dataSize == sEmptySubRecordSize)
            fail("zero-sized " + std::string(field));
        if (!reader.getZString(value))
    ...
    while (reader.getSubRecordHeader())
    {
        ...
        if (menu == nullptr)
        {
            switch (header.typeId)
            {
                ...
                case ESM::fourCC("OBND"):
                    ...
                    requireSize(header, ESM4::Terminal::sObjectBoundsSerializedSize);
                ...
                case ESM::fourCC("MODT"):
                    ...
                    requireOneOfSizes(header,
                        { sModelDataSizeSmall, sModelDataSizeMedium, sModelDataSizeLarge, sModelDataSizeLargest });
                ...
                case ESM::fourCC("MODS"):
                    ...
                    requireOneOfSizes(header,
                        { sModelTextureSwapsSize, sModelTextureSwapsWithTerminatorSize,
                            sModelTextureSwapsLargeSize });
                ...
                case ESM::fourCC("SCRI"):
                    ...
                    requireSize(header, sFormIdSerializedSize);
                ...
                case ESM::fourCC("DNAM"):
                    ...
                    requireOneOfSizes(header, { sDataSizeLegacy, sDataSizeCurrent });
        ...
        switch (header.typeId)
        {
            ...
            case ESM::fourCC("ANAM"):
                ...
                requireSize(header, sizeof(menu->mFlags));
                readExact(reader, menu->mFlags, "ANAM");
                if (menu->mFlags > sMaxMenuFlags)
            ...
            case ESM::fourCC("SCHR"):
                ...
                if (menu->mScript.scriptHeader.unused != sEmptySubRecordSize)
                    fail("unsupported nonzero SCHR unused field");
                if (menu->mScript.scriptHeader.type > sMaxScriptType)
                    fail("unsupported SCHR type " + std::to_string(menu->mScript.scriptHeader.type));
                if (menu->mScript.scriptHeader.flag > sMaxScriptFlags)
                    fail("unsupported SCHR flags " + std::to_string(menu->mScript.scriptHeader.flag));
                menuPhase = MenuPhase::AfterHeader;
                break;
            case ESM::fourCC("SCDA"):
                if (menuPhase != MenuPhase::AfterHeader
                    || menu->mScript.scriptHeader.compiledSize == sEmptySubRecordSize)
                ...
                readBytes(reader, menu->mCompiledData, "SCDA");
            ...
            case ESM::fourCC("SCTX"):
                ...
                if (header.dataSize == sEmptySubRecordSize || !reader.getString(menu->mScript.scriptSource))
            ...
            case ESM::fourCC("SLSD"):
            {
                ...
                requireSize(header, sScriptLocalVariableSerializedSize);
            ...
            case ESM::fourCC("SCRO"):
            {
                ...
                menu->mReferences.push_back(reference);
                menuPhase = MenuPhase::FormReferences;
                break;
            }
            case ESM::fourCC("CTDA"):
