...
void ESM4::ActorValueInfo::load(ESM4::Reader& reader)
{
    ...
    while (reader.getSubRecordHeader())
    {
        ...
        switch (subHdr.typeId)
        {
            ...
            case ESM::fourCC("AVSK"):
                mProgressionMarkerTypes.push_back(subHdr.typeId);
                mProgressionMarkers.push_back(readRawSubrecord(reader, subHdr.dataSize));
                break;
            default:
                throw std::runtime_error("ESM4::AVIF::load - Unknown subrecord " + ESM::printName(subHdr.typeId));
        }
    }
}
