...
namespace
{
    ...
    TEST(Esm4TerminalTest, preservesExactRetailBytecodeSourceLocalsReferencesAndConditions)
    {
        ...
        EXPECT_EQ(scriptItem.mCompiledData,
            (std::vector<std::uint8_t>(bytecode.begin(), bytecode.end())));
        EXPECT_EQ(scriptItem.mScript.scriptSource, source);
        ASSERT_EQ(scriptItem.mReferences.size(), 3);
        EXPECT_EQ(scriptItem.mReferences[0], ESM::FormId::fromUint32(0x050029f0));
        EXPECT_EQ(scriptItem.mReferences[1], ESM::FormId::fromUint32(0x050029f1));
        EXPECT_EQ(scriptItem.mReferences[2], ESM::FormId::fromUint32(0x05012f50));
