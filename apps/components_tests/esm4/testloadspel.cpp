...
namespace
{
    ...
    constexpr std::array<std::uint8_t, ESM4::Spell::sDataSerializedSize> sPlasmaSpellData{
        // FalloutNV.esm 00092C48 PlasmaEffect SPEL.SPIT.
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x6a, 0xcd, 0xcd, 0xcd,
    };
    constexpr std::array<std::uint8_t, ESM4::Spell::sEffectSerializedSize> sPlasmaEffectData{
    ...
    TEST(Esm4SpellTest, loadsAndAdjustsRetailCriticalSpellReferences)
    {
        ...
        condition.condition = ESM4::CTF_Combine;
        condition.functionIndex = ESM4::FUN_GetQuestRunning;
        condition.param1 = 0x1234;
        condition.reference = 0x5678;
        appendSubRecord(payload, "CTDA", std::string_view(
            reinterpret_cast<const char*>(&condition), sizeof(condition)));
        ESM4::TargetCondition nullCondition{};
        nullCondition.functionIndex = ESM4::FUN_GetQuestRunning;
        appendSubRecord(payload, "CTDA", std::string_view(
            reinterpret_cast<const char*>(&nullCondition), sizeof(nullCondition)));
        ...
        ASSERT_EQ(spell.mEffects.front().conditions.size(), 2u);
        EXPECT_EQ(spell.mEffects.front().conditions.front().param1, 0x02001234u);
        EXPECT_EQ(spell.mEffects.front().conditions.front().reference, 0x02005678u);
        EXPECT_EQ(spell.mEffects.front().conditions.back().param1, 0u);
        EXPECT_EQ(spell.mEffects.front().conditions.back().reference, 0u);
    }
