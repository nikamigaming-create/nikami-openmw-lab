...
namespace
{
    ...
    constexpr std::array<std::uint8_t, ESM4::MagicEffect::sDataSerializedSize> sGooificationData{
    ...
    TEST(Esm4MagicEffectTest, loadsAndAdjustsRetailCriticalEffectReferences)
    {
        ...
        EXPECT_TRUE(effect.mData.light.isZeroOrUnset());
        EXPECT_TRUE(effect.mData.effectShader.isZeroOrUnset());
        EXPECT_TRUE(effect.mData.objectDisplayShader.isZeroOrUnset());
        EXPECT_TRUE(effect.mData.effectSound.isZeroOrUnset());
        EXPECT_TRUE(effect.mData.boltSound.isZeroOrUnset());
        EXPECT_TRUE(effect.mData.areaSound.isZeroOrUnset());
    }
    ...
    TEST(Esm4MagicEffectTest, rejectsEveryNonRetailDataSizeAndInvalidArchetypeWithoutMutation)
    {
        ESM4::MagicEffect::Data data;
        data.flags = 42;
        const std::array<std::uint8_t, ESM4::MagicEffect::sDataSerializedSize - sizeof(std::uint32_t)> shortData{};
        const std::array<std::uint8_t, ESM4::MagicEffect::sDataSerializedSize + sizeof(std::uint32_t)> longData{};
        ...
        std::memcpy(invalid.data() + ESM4::MagicEffect::sDataArchetypeOffset, &invalidArchetype,
            sizeof(invalidArchetype));
