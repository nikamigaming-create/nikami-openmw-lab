...
namespace
{
    ...
    {
        // Fallout4VR.esm 00249369 PROJ.DNAM (VATSBullet45cal_Explosive).
        const std::array<std::uint8_t, 93> bytes{
            ...
            0x67, 0x87, 0x08, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00,
        };
        ESM4::Projectile::Data data;
        ...
        EXPECT_EQ(std::bit_cast<std::uint32_t>(data.coneSpread), std::uint32_t{ 0 });
        EXPECT_EQ(std::bit_cast<std::uint32_t>(data.collisionRadius), std::uint32_t{ 0x40000000 });
        EXPECT_EQ(std::bit_cast<std::uint32_t>(data.lifetime), std::uint32_t{ 0 });
        EXPECT_EQ(std::bit_cast<std::uint32_t>(data.relaunchInterval), std::uint32_t{ 0x3e800000 });
        EXPECT_EQ(data.decalData.toUint32(), 0);
        EXPECT_EQ(data.collisionLayer.toUint32(), 0x88767);
        EXPECT_EQ(data.tracerFrequency, 1);
