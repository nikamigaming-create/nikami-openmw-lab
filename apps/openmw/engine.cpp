...
// The DAO compatibility reader is linked through openmw-lib. Its only public
// entry point is OSG's static plug-in registration symbol, so make that symbol
// an explicit dependency; otherwise MSVC drops ReaderWriterGLTF.obj and .glb
// files misleadingly fail as "file not handled" at runtime.
USE_OSGPLUGIN(gltf)
...
namespace
{
    class ProofPostProcessCaptureCallback final : public osg::GraphicsContext::SwapCallback
    {
    public:
        explicit ProofPostProcessCaptureCallback(
            osgViewer::ScreenCaptureHandler::CaptureOperation* operation,
            osg::GraphicsContext::SwapCallback* previous)
            : mOperation(operation)
            , mPrevious(previous)
        {
        }

        void arm() { mArmed.store(true); }

        void swapBuffersImplementation(osg::GraphicsContext* graphicsContext) override
        {
            if (mPrevious != nullptr)
                mPrevious->swapBuffersImplementation(graphicsContext);
            else if (graphicsContext != nullptr)
                graphicsContext->swapBuffersImplementation();
            if (mArmed.exchange(false) && mOperation != nullptr && graphicsContext != nullptr)
            {
                ...
                if (traits != nullptr)
                {
                    ...
                    image->readPixels(0, 0, traits->width, traits->height, GL_RGB, GL_UNSIGNED_BYTE);
                    (*mOperation)(*image, graphicsContext->getState()->getContextID());
                }
            }
        }

    private:
        osg::ref_ptr<osgViewer::ScreenCaptureHandler::CaptureOperation> mOperation;
        osg::ref_ptr<osg::GraphicsContext::SwapCallback> mPrevious;
        mutable std::atomic_bool mArmed{ false };
    };
    ...
    private:
        ...
        static void applyHairState(osg::StateSet* stateSet)
        {
            ...
            stateSet->setAttributeAndModes(
                ...
                protectedOverride);
            stateSet->setAttributeAndModes(new osg::AlphaFunc(osg::AlphaFunc::GREATER, 0.015f), protectedOverride);
        }
    };
    ...
    class ProofBackgroundStateVisitor final : public osg::NodeVisitor
    {
    public:
        ProofBackgroundStateVisitor()
    ...
    private:
        static void applyState(osg::StateSet* stateSet)
        {
            constexpr unsigned int protectedOverride
                = osg::StateAttribute::ON | osg::StateAttribute::OVERRIDE | osg::StateAttribute::PROTECTED;
            // Draw an authored sky layer before normal world geometry. It never
            // writes depth, so terrain, water, props, and actors remain fully
            // authoritative even when the OBJ loader classifies a PNG sky as a
            // transparent late-bin material.
            stateSet->setMode(GL_DEPTH_TEST, protectedOverride);
            stateSet->setAttributeAndModes(
                new osg::Depth(osg::Depth::ALWAYS, 0.0, 1.0, false), protectedOverride);
            stateSet->setMode(GL_BLEND, osg::StateAttribute::OFF | osg::StateAttribute::OVERRIDE
                    | osg::StateAttribute::PROTECTED);
            // The authored cloud dome is already color-composed. Keep it out
            // of the terrain/town light equation so matching scene exposure
            // does not also crush the sky.
            stateSet->setMode(GL_LIGHTING, osg::StateAttribute::OFF | osg::StateAttribute::OVERRIDE
                    | osg::StateAttribute::PROTECTED);
            osg::ref_ptr<osg::Program> skyProgram = new osg::Program;
            skyProgram->setName("OpenDAO authored cloud atmosphere");
            skyProgram->addShader(new osg::Shader(osg::Shader::VERTEX, R"glsl(
#version 120
varying vec2 daoSkyUv;
varying float daoSkyHeight;
void main()
{
    gl_Position = ftransform();
    daoSkyUv = gl_MultiTexCoord0.xy;
    daoSkyHeight = clamp((gl_Vertex.z + 100.0) / 700.0, 0.0, 1.0);
}
)glsl"));
            skyProgram->addShader(new osg::Shader(osg::Shader::FRAGMENT, R"glsl(
#version 120
uniform sampler2D daoSkyClouds;
varying vec2 daoSkyUv;
varying float daoSkyHeight;
void main()
{
    // The source is a compact tiling cloud field, not a finished panorama.
    // Combine three authored octaves so it remains legible at 1080p instead
    // of collapsing into a uniform grey dome after minification.
    float low = texture2D(daoSkyClouds, daoSkyUv * 2.0 + vec2(0.17, 0.04)).r;
    float mid = texture2D(daoSkyClouds, daoSkyUv * 5.0 + vec2(0.41, 0.23)).r;
    float high = texture2D(daoSkyClouds, daoSkyUv * 11.0 + vec2(0.08, 0.57)).r;
    float density = low * 0.58 + mid * 0.30 + high * 0.12;
    vec3 horizon = vec3(0.46, 0.45, 0.47);
    vec3 zenith = vec3(0.11, 0.16, 0.27);
    vec3 sky = mix(horizon, zenith, smoothstep(0.0, 0.92, daoSkyHeight));
    float clouds = smoothstep(0.38, 0.68, density);
    vec3 cloudColor = mix(vec3(0.24, 0.25, 0.29), vec3(0.82, 0.79, 0.73), density);
    gl_FragColor = vec4(mix(sky, cloudColor, clouds * 0.82), 1.0);
}
)glsl"));
            stateSet->setAttributeAndModes(skyProgram,
                osg::StateAttribute::ON | osg::StateAttribute::OVERRIDE | osg::StateAttribute::PROTECTED);
            stateSet->addUniform(new osg::Uniform("daoSkyClouds", 0));
            // The DAO atmosphere mesh is an outward-wound sphere and the
            // viewer is deliberately placed inside it. Render both sides so
            // the authored cloud texture is visible from the playable side.
    ...
    void forceProofBackgroundState(osg::Node& node)
    {
        ProofBackgroundStateVisitor visitor;
        node.accept(visitor);
    }
    ...
    // An opt-in route runner used only for unattended compatibility evidence. Routes are
    // declared outside the game data, so this runner has no knowledge of a particular
    // campaign, quest, cell, or reference. It drives normal World movement and activation
    // APIs, while deliberately avoiding all desktop/window input.
                ...
                else
                {
                    ...
                    else if (step.mOperation == "move-to-reference")
                    {
                        ...
                        else
                        {
                            ...
                            completed = true;
                            state.mDetail = "moved player to " + reference.getCellRef().getRefNum().toString("FormId:");
                        }
                    }
                    else if (step.mOperation == "activate-reference")
                    {
                        ...
                        if (!player.isEmpty())
                        {
                            // The declared route crosses the same engine-level activation boundary as normal play.
                            // It must not bypass the placed reference's authored OnActivate script.
    ...
    private:
        ...
        void writeReport(unsigned frameNumber) const
        {
            ...
            writeProofJsonString(output, mRouteId);
            output << ",\n  \"routePath\": ";
            writeProofJsonString(output, mRoutePath.string());
            output << ",\n  \"status\": \"" << (mPassed ? "pass" : "fail") << "\",\n  \"frame\": "
                   << frameNumber << ",\n  \"resultDetail\": ";
            writeProofJsonString(output, mResultDetail);
            ...
            for (std::size_t index = 0; index < mSteps.size(); ++index)
            {
                ...
                output << "\n    {\"id\":";
                writeProofJsonString(output, step.mId);
                output << ",\"operation\":";
                writeProofJsonString(output, step.mOperation);
                output << ",\"status\":";
                writeProofJsonString(output, compatibilityRouteStepStatus(state.mCompleted, state.mFailed, state.mStarted));
                output << ",\"startFrame\":" << state.mStartFrame << ",\"finishFrame\":" << state.mFinishFrame
                       << ",\"detail\":";
                writeProofJsonString(output, state.mDetail);
                output << '}';
            }
            output << "\n  ]\n}\n";
        }
    ...
    std::string daoJsonEscape(std::string_view value)
    {
    ...
    void writeDaoMatrix(std::ostream& out, const osg::Matrixd& matrix)
    {
        out << "[";
    ...
    void writeDaoStateSet(std::ostream& out, const osg::StateSet* stateSet)
    {
        out << "{\"blend\":" << (stateSet != nullptr && stateSet->getMode(GL_BLEND) != 0 ? "true" : "false")
            << ",\"alpha_test\":"
            << (stateSet != nullptr && stateSet->getMode(GL_ALPHA_TEST) != 0 ? "true" : "false")
            << ",\"lighting\":"
            << (stateSet != nullptr && stateSet->getMode(GL_LIGHTING) != 0 ? "true" : "false")
            << ",\"textures\":[";
        ...
        if (stateSet != nullptr)
        {
            for (unsigned unit = 0; unit < 16; ++unit)
            {
                ...
                out << "{\"unit\":" << unit << ",\"file\":\""
                    << daoJsonEscape(image != nullptr ? image->getFileName() : std::string()) << "\",\"width\":"
                    << (image != nullptr ? image->s() : 0) << ",\"height\":"
                    << (image != nullptr ? image->t() : 0) << "}";
            }
        }
        out << "]";
        const auto* material = stateSet != nullptr
        ...
        if (material != nullptr)
        {
            ...
            out << ",\"material\":{\"diffuse\":[" << diffuse.r() << "," << diffuse.g() << ","
                << diffuse.b() << "," << diffuse.a() << "],\"ambient\":[" << ambient.r() << ","
                << ambient.g() << "," << ambient.b() << "," << ambient.a() << "]}";
        }
        out << "}";
    }

    class DaoRuntimeTelemetryVisitor : public osg::NodeVisitor
    {
    public:
        DaoRuntimeTelemetryVisitor(std::ostream& nodes, std::ostream& surfaces, std::string layer)
            : osg::NodeVisitor(TRAVERSE_ALL_CHILDREN)
            , mNodes(nodes)
            , mSurfaces(surfaces)
        ...
        void apply(osg::Node& node) override
        {
            ...
            mNodes << "{\"layer\":\"" << daoJsonEscape(mLayer) << "\",\"name\":\""
                   << daoJsonEscape(node.getName()) << "\",\"class\":\"" << daoJsonEscape(node.className())
                   << "\",\"mask\":" << node.getNodeMask() << ",\"bound\":[" << bound.center().x() << ","
                   << bound.center().y() << "," << bound.center().z() << "," << bound.radius()
                   << "],\"world_matrix\":";
            writeDaoMatrix(mNodes, osg::computeLocalToWorld(getNodePath()));
            mNodes << ",\"state\":";
            writeDaoStateSet(mNodes, node.getStateSet());
            mNodes << "}";

            if (auto* geode = node.asGeode())
            {
                for (unsigned index = 0; index < geode->getNumDrawables(); ++index)
                {
                    ...
                    if (const auto* geometry = drawable != nullptr ? drawable->asGeometry() : nullptr)
                        vertices = geometry->getVertexArray() != nullptr ? geometry->getVertexArray()->getNumElements() : 0;
                    mSurfaces << "{\"layer\":\"" << daoJsonEscape(mLayer) << "\",\"node\":\""
                              << daoJsonEscape(node.getName()) << "\",\"index\":" << index
                              << ",\"vertices\":" << vertices << ",\"state\":";
                    writeDaoStateSet(mSurfaces, drawable != nullptr ? drawable->getStateSet() : nullptr);
                    mSurfaces << "}";
                }
            }
            traverse(node);
    ...
    private:
        std::ostream& mNodes;
        std::ostream& mSurfaces;
        std::string mLayer;
        bool mFirstNode = true;
        bool mFirstSurface = true;
    };

    bool writeDaoRuntimeTelemetry(const std::filesystem::path& path, osg::Node* scene, osg::Node* actors,
        const osg::Camera* camera, std::string_view scenePaths, std::string_view actorPath)
    {
        ...
        DaoRuntimeTelemetryVisitor sceneVisitor(nodes, surfaces, "scene");
        if (scene != nullptr)
            scene->accept(sceneVisitor);
        std::ostringstream actorNodes;
        std::ostringstream actorSurfaces;
        DaoRuntimeTelemetryVisitor actorVisitor(actorNodes, actorSurfaces, "actors");
        if (actors != nullptr)
            actors->accept(actorVisitor);
        out << "{\"schema\":\"opendao-runtime-state-v1\",\"engine\":\"openmw\",\"scene_paths\":\""
            << daoJsonEscape(scenePaths) << "\",\"actor_path\":\"" << daoJsonEscape(actorPath)
            << "\",\"camera\":{";
        if (camera != nullptr)
        {
            out << "\"view\":";
            writeDaoMatrix(out, camera->getViewMatrix());
            out << ",\"projection\":";
            writeDaoMatrix(out, camera->getProjectionMatrix());
        }
        out << "},\"counts\":{\"nodes\":" << sceneVisitor.mNodeCount + actorVisitor.mNodeCount
            << ",\"surfaces\":" << sceneVisitor.mSurfaceCount + actorVisitor.mSurfaceCount
            << "},\"nodes\":[" << nodes.str()
            << (sceneVisitor.mNodeCount != 0 && actorVisitor.mNodeCount != 0 ? "," : "")
            << actorNodes.str() << "],\"surfaces\":[" << surfaces.str()
            << (sceneVisitor.mSurfaceCount != 0 && actorVisitor.mSurfaceCount != 0 ? "," : "")
            << actorSurfaces.str() << "]}";
        return true;
    }
    ...
    void applyWorldViewerStaticCamera(MWRender::Camera* camera, const osg::Vec3d& eye, const osg::Vec3d& target)
    {
        ...
        const osg::Vec3d forward = camera->getOrient() * osg::Vec3d(0.0, 1.0, 0.0);
        Log(Debug::Info) << "World viewer exact-look audit eye=(" << eye.x() << "," << eye.y() << ","
                         << eye.z() << ") target=(" << target.x() << "," << target.y() << ","
                         << target.z() << ") requestedForward=(" << delta.x() / delta.length() << ","
                         << delta.y() / delta.length() << "," << delta.z() / delta.length()
                         << ") actualForward=(" << forward.x() << "," << forward.y() << ","
                         << forward.z() << ")";
    }
    ...
    // Export the exact post-skinning geometry OpenMW has already assembled for
    // an FNV actor.  Godot consumes these authored snapshots; it never guesses
    // body parts or substitutes proxy characters.
    ...
    private:
        void writeGeometry(osg::Geometry& geometry, osg::Drawable& drawable, const osg::Matrixd& world)
        {
            ...
            const auto imageFromState = [](const osg::StateSet* state) -> const osg::Image* {
                if (state == nullptr)
                    return nullptr;
            ...
            const osg::Image* image = imageFromState(drawable.getStateSet());
            if (image == nullptr)
                image = imageFromState(geometry.getStateSet());
            if (image == nullptr)
            {
                const osg::NodePath& path = getNodePath();
                for (auto it = path.rbegin(); it != path.rend() && image == nullptr; ++it)
                    image = *it == nullptr ? nullptr : imageFromState((*it)->getStateSet());
            }
            if (image != nullptr && !image->getFileName().empty())
            {
                std::string imagePath = image->getFileName();
                std::replace(imagePath.begin(), imagePath.end(), '\\', '/');
                mMtl << "map_Kd " << imagePath << '\n';
            }
            mMtl << '\n';
...
#include "opennvactorskeletonexport.inc"
...
bool OMW::Engine::frame(unsigned frameNumber, float frametime)
{
    ...
    try
    {
        ...
        // https://github.com/MyGUI/mygui/issues/21)
        {
            ...
            if (!mWindowManager->isWindowVisible() && !backgroundPlayableSession)
            {
            ...
            if (!mWindowManager->isWindowVisible() && backgroundPlayableSession && !backgroundPlayableSessionLogged)
            {
    ...
    // Imported DAO GLBs are not Bullet objects yet. Keep the interactive
    // world-viewer player controllable instead of trapping it in the hidden
    // Morrowind host cell; this flag is removed once the GLB collision bridge
    // is active.
    static bool proofWorldViewerNoclipApplied = false;
    if (!proofWorldViewerNoclipApplied && proofWorldReady && mWorld != nullptr
        && proofEnvEnabled("OPENMW_WORLD_VIEWER_NOCLIP"))
    {
        mWorld->toggleCollisionMode();
        proofWorldViewerNoclipApplied = true;
        Log(Debug::Info) << "World viewer: enabled temporary noclip pending imported GLB collision";
    }
    ...
    static osg::ref_ptr<osg::Camera> proofExtraSceneCamera;
    static osg::ref_ptr<osg::MatrixTransform> proofExtraOuterScene;
    static osg::ref_ptr<osg::Camera> proofExtraOuterSceneCamera;
    if (proofWorldReady && mWorld != nullptr && !proofExtraSceneAttempted)
    {
        ...
        const char* extraSceneManifest = std::getenv("OPENMW_WORLD_VIEWER_EXTRA_SCENE_MANIFEST");
        std::string extraSceneSpec = extraScenePath != nullptr ? extraScenePath : "";
        if (extraSceneManifest != nullptr && *extraSceneManifest != '\0')
        {
            std::ifstream manifest(extraSceneManifest);
            std::string line;
            ...
            while (std::getline(manifest, line))
            {
                ...
                if (!manifestSpec.empty())
                    manifestSpec.push_back(';');
                manifestSpec += line;
            }
            if (!manifestSpec.empty())
            {
                if (!extraSceneSpec.empty())
                    manifestSpec.push_back(';');
                extraSceneSpec = manifestSpec + extraSceneSpec;
            }
            Log(Debug::Info) << "World viewer extra scene manifest path=" << extraSceneManifest
                             << " entries=" << std::count(extraSceneSpec.begin(), extraSceneSpec.end(), ';') + 1;
        }
        if (!extraSceneSpec.empty())
        {
            proofExtraSceneAttempted = true;
            osg::ref_ptr<osg::Group> sceneGroup = new osg::Group;
            osg::ref_ptr<osg::Group> outerSceneGroup = new osg::Group;
            // A Haven area manifest may place the same tree or prop GLB
            // thousands of times. osgDB::readRefNodeFile does not guarantee a
            // shared node for these direct filesystem loads, so reading on
            // every placement duplicates meshes and embedded textures until a
            // complete DAO area consumes tens of gigabytes. OSG nodes support
            // multiple parents; retain one immutable asset root and attach it
            // beneath a unique placement transform for each instance.
            std::unordered_map<std::string, osg::ref_ptr<osg::Node>> assetCache;
            std::size_t assetCacheHits = 0;
            std::istringstream scenePaths(extraSceneSpec);
            std::string scenePath;
            while (std::getline(scenePaths, scenePath, ';'))
            {
                if (scenePath.empty())
                    continue;
                // Optional per-asset DAO placement suffix: path|x,y,z. This
                // keeps the original GLB byte-for-byte intact and applies the
                // Haven area-table transform in OpenMW instead of baking it
                // through Blender (which changes the terrain axis convention).
                std::string assetPath = scenePath;
                osg::Vec3f assetOffset(0.f, 0.f, 0.f);
                osg::Quat assetRotation;
                float assetScale = 1.f;
                bool hasAssetOffset = false;
                bool hasAssetRotation = false;
                const std::size_t placementSeparator = scenePath.rfind('|');
                if (placementSeparator != std::string::npos)
                {
                    assetPath = scenePath.substr(0, placementSeparator);
                    std::string placement = scenePath.substr(placementSeparator + 1);
                    std::replace(placement.begin(), placement.end(), ',', ' ');
                    std::istringstream placementValues(placement);
                    hasAssetOffset = static_cast<bool>(
                        placementValues >> assetOffset.x() >> assetOffset.y() >> assetOffset.z());
                    float qx = 0.f;
                    float qy = 0.f;
                    float qz = 0.f;
                    float qw = 1.f;
                    if (hasAssetOffset && placementValues >> qx >> qy >> qz >> qw)
                    {
                        assetRotation.set(qx, qy, qz, qw);
                        hasAssetRotation = true;
                        placementValues >> assetScale;
                    }
                }
                osg::ref_ptr<osg::Node> sceneNode;
                const auto cachedAsset = assetCache.find(assetPath);
                if (cachedAsset != assetCache.end())
                {
                    sceneNode = cachedAsset->second;
                    ++assetCacheHits;
                }
                else
                {
                    sceneNode = osgDB::readRefNodeFile(assetPath);
                    if (sceneNode != nullptr)
                        assetCache.emplace(assetPath, sceneNode);
                }
                if (sceneNode == nullptr)
                    Log(Debug::Error) << "World viewer extra scene: failed to load path=" << assetPath;
                else
                {
                    if (hasAssetOffset)
                    {
                        osg::ref_ptr<osg::MatrixTransform> placement = new osg::MatrixTransform;
                        placement->setName("OpenDAO authored instance " + assetPath);
                        placement->setMatrix(osg::Matrixf::scale(assetScale, assetScale, assetScale)
                            * osg::Matrixf::rotate(hasAssetRotation ? assetRotation : osg::Quat())
                            * osg::Matrixf::translate(assetOffset));
                        placement->addChild(sceneNode);
                        sceneNode = placement;
                    }
                    // Godot's authored Redcliffe composition is the near-field
                    // town layer; connected terrain cells are streamed around
                    // it as context. Preserve that composition boundary in the
                    // compatibility camera so a later terrain draw cannot paint
                    // its skirts over nearer buildings and roads.
                    if (assetPath.find("redcliffe-environment-v2.glb") != std::string::npos)
                    {
                        osg::StateSet* composedTownState = sceneNode->getOrCreateStateSet();
                        constexpr unsigned int protectedOverride = osg::StateAttribute::ON
                            | osg::StateAttribute::OVERRIDE | osg::StateAttribute::PROTECTED;
                        composedTownState->setAttributeAndModes(
                            new SceneUtil::AutoDepth(osg::Depth::LEQUAL, 0.0, 1.0, true), protectedOverride);
                        composedTownState->setMode(GL_DEPTH_TEST, protectedOverride);
                        composedTownState->setRenderBinDetails(
                            10, "RenderBin", osg::StateSet::OVERRIDE_PROTECTED_RENDERBIN_DETAILS);
                        composedTownState->setNestRenderBins(false);
                    }
                    const osg::BoundingSphere bound = sceneNode->getBound();
                    Log(Debug::Info) << "World viewer extra scene child path=" << assetPath
                                     << " offset=" << assetOffset
                                     << " boundCenter=" << bound.center() << " boundRadius=" << bound.radius();
                    const bool isBackgroundChild = sceneGroup->getNumChildren() == 0
                        && outerSceneGroup->getNumChildren() == 0
                        && proofEnvEnabled("OPENMW_WORLD_VIEWER_EXTRA_SCENE_BACKGROUND_FIRST");
                    if (isBackgroundChild
                        && assetPath.find("marethari-camp") == std::string::npos)
                    {
                        forceProofBackgroundState(*sceneNode);
                    }
                    ...
                    if (sceneGroup->getNumChildren() > 0
                        && proofEnvEnabled("OPENMW_WORLD_VIEWER_EXTRA_SCENE_DAO_ALPHA_CUTOUT")
                        && assetPath.find(".glb") == std::string::npos
                        && assetPath.find(".gltf") == std::string::npos)
                    {
                        osg::StateSet* childState = sceneNode->getOrCreateStateSet();
                        childState->setMode(
                            GL_BLEND, osg::StateAttribute::OFF | osg::StateAttribute::OVERRIDE);
                        const bool authoredTownLayer
                            = assetPath.find("environment-openmw-native") != std::string::npos
                            || assetPath.find("setpieces") != std::string::npos;
                        osg::ref_ptr<SceneUtil::AutoDepth> depth = new SceneUtil::AutoDepth(
                            authoredTownLayer ? osg::Depth::ALWAYS : osg::Depth::LEQUAL, 0.0, 1.0, true);
                        childState->setAttributeAndModes(
                            depth, osg::StateAttribute::ON | osg::StateAttribute::OVERRIDE);
                        childState->setMode(
                            GL_DEPTH_TEST, osg::StateAttribute::ON | osg::StateAttribute::OVERRIDE);
                        childState->setRenderingHint(osg::StateSet::OPAQUE_BIN);
                        const bool foliage = assetPath.find("vegetation") != std::string::npos;
                        if (foliage)
                        {
                            osg::ref_ptr<osg::AlphaFunc> alpha = new osg::AlphaFunc;
                            alpha->setFunction(osg::AlphaFunc::GREATER, 0.28f);
                            childState->setAttributeAndModes(
                                alpha, osg::StateAttribute::ON | osg::StateAttribute::OVERRIDE);
                            childState->setMode(
                                GL_ALPHA_TEST, osg::StateAttribute::ON | osg::StateAttribute::OVERRIDE);
                        }
                        else
                        {
                            childState->setMode(
                    ...
                    // A placement suffix is a transform, not a rendering-layer
                    // declaration. Keep positioned DAO assets in the shared
                    // depth scene unless the caller explicitly requests the
                    // legacy connected-terrain outer pass.
                    const bool offsetUsesOuterPass = hasAssetOffset
                        && proofEnvEnabled("OPENMW_WORLD_VIEWER_EXTRA_SCENE_OFFSET_AS_OUTER");
                    if (offsetUsesOuterPass || isBackgroundChild)
                        outerSceneGroup->addChild(sceneNode);
                    else
                        sceneGroup->addChild(sceneNode);
                }
            }
            Log(Debug::Info) << "World viewer extra scene asset cache unique=" << assetCache.size()
                             << " hits=" << assetCacheHits;
            osg::ref_ptr<osg::Node> node = sceneGroup;
            if (sceneGroup->getNumChildren() == 0)
            {
                Log(Debug::Error) << "World viewer extra scene: failed to load path=" << extraSceneSpec;
            }
            else
            {
                ...
                if (outerSceneGroup->getNumChildren() > 0)
                {
                    proofExtraOuterScene = new osg::MatrixTransform;
                    proofExtraOuterScene->setName("OpenDAO connected terrain pass");
                    proofExtraOuterScene->setMatrix(proofExtraScene->getMatrix());
                    proofExtraOuterScene->addChild(outerSceneGroup);
                    osg::StateSet* outerState = proofExtraOuterScene->getOrCreateStateSet();
                    outerState->setAttributeAndModes(
                        new SceneUtil::AutoDepth(osg::Depth::LEQUAL, 0.0, 1.0, true),
                        osg::StateAttribute::ON | osg::StateAttribute::OVERRIDE);
                    outerState->setMode(
                        GL_DEPTH_TEST, osg::StateAttribute::ON | osg::StateAttribute::OVERRIDE);
                }

                // The imported compatibility scene is attached beside native
                // OpenMW cells rather than through a cell drawable, so it
                // cannot rely on a cell root to provide depth state. Without
                // this, later terrain primitives paint over nearer buildings
                // according to submission order. The authored sky child uses
                // PROTECTED depth-always/no-write state and remains behind it.
                const bool sharedMainCamera
                    = proofEnvEnabled("OPENMW_WORLD_VIEWER_EXTRA_SCENE_SHARED_MAIN_CAMERA");
                if (proofEnvEnabled("OPENMW_WORLD_VIEWER_EXTRA_SCENE_DAO_PBR"))
                {
                    osg::StateSet* daoSceneState = proofExtraScene->getOrCreateStateSet();
                    daoSceneState->setMode(
                        GL_DEPTH_TEST, osg::StateAttribute::ON | osg::StateAttribute::OVERRIDE);
                    daoSceneState->setAttributeAndModes(
                        new SceneUtil::AutoDepth(osg::Depth::LEQUAL, 0.0, 1.0, true),
                        osg::StateAttribute::ON | osg::StateAttribute::OVERRIDE);
                }
                ...
                else if (proofEnvEnabled("OPENMW_WORLD_VIEWER_EXTRA_SCENE_PROOF_LIGHT"))
                {
                    ...
                    // Native DAO glTF materials retain their authored winding.
                    // Disabling culling globally exposes terrain/roof backfaces
                    // from the lake and makes a correct scene look folded.
                    if (!proofEnvEnabled("OPENMW_WORLD_VIEWER_EXTRA_SCENE_DAO_PBR"))
                        stateSet->setMode(
                            GL_CULL_FACE, osg::StateAttribute::OFF | osg::StateAttribute::OVERRIDE);
                    osg::ref_ptr<osg::Light> light = new osg::Light;
                    light->setLightNum(7);
                    const char* daoFaceCharacter = std::getenv("OPENMW_DAO_FACE_CHARACTER");
                    const bool marethariDialogue = daoFaceCharacter != nullptr
                        && std::string(daoFaceCharacter) == "keeper_marethari";
                    if (proofEnvEnabled("OPENMW_WORLD_VIEWER_EXTRA_SCENE_DAO_ATMOSPHERE"))
                    {
                        // Redcliffe lak100d authored outdoor environment:
                        // sun=(-0.713618,-0.33573,-0.614845), colour=(0.72,0.585,0.369).
                        if (marethariDialogue)
                        {
                            // Same warm dialogue key as the accepted Godot
                            // oracle: camera/target lerp(.28) + 0.38 m up.
                            light->setPosition(osg::Vec4f(
                                257.485378f, 270.429846f, 4.704605f, 1.f));
                            light->setConstantAttenuation(1.f);
                            light->setLinearAttenuation(1.f / 4.5f);
                            light->setQuadraticAttenuation(0.f);
                        }
                        else
                            light->setPosition(osg::Vec4f(0.713618f, 0.33573f, 0.614845f, 0.f));
                        // DAO's exterior composition is deliberately low-key. A
                        // Morrowind-style bright ambient fill flattens its brown/
                        // green terrain and makes the village read grey.
                        // Match the retained Godot oracle's measured ground
                        // luminance (0.137 versus the old fixed-pipeline 0.310).
                        // brc997d.havenarea ambient/fog colour
                        // (0.83, 0.70, 0.33) at Godot energy 0.65.
                        light->setAmbient(osg::Vec4f(0.5395f, 0.455f, 0.2145f, 1.f));
                        light->setDiffuse(marethariDialogue
                                ? osg::Vec4f(1.4f, 1.148f, 0.98f, 1.f)
                                : osg::Vec4f(0.42f, 0.345f, 0.212f, 1.f));
                        light->setSpecular(marethariDialogue
                                ? osg::Vec4f(0.35f, 0.287f, 0.245f, 1.f)
                                : osg::Vec4f(0.10f, 0.083f, 0.054f, 1.f));

                        osg::ref_ptr<osg::Fog> fog = new osg::Fog;
                        // Godot runtime telemetry has volumetric fog enabled
                        // with density 0.01, length 320 m, and this blue-grey
                        // scattering colour. EXP2 is OpenGL's closest native
                        // approximation to that volumetric extinction curve.
                        fog->setMode(osg::Fog::EXP2);
                        fog->setDensity(0.01f);
                        fog->setColor(osg::Vec4f(0.518f, 0.553f, 0.608f, 1.f));
                        fog->setStart(0.f);
                        fog->setEnd(320.f);
                        stateSet->setAttributeAndModes(
                            fog, osg::StateAttribute::ON | osg::StateAttribute::OVERRIDE);
                        stateSet->setMode(GL_FOG, osg::StateAttribute::ON | osg::StateAttribute::OVERRIDE);
                    }
                    else
                    {
                    ...
                    osg::ref_ptr<osg::Node> litNode = node;
                    if (marethariDialogue)
                    {
                        // The portrait key occupies slot 7. Keep the Dalish
                        // area's authored directional light in slot 6 so the
                        // GLSL path receives both lights in eye space, just as
                        // Godot's DirectionalLight3D + OmniLight3D does.
                        osg::ref_ptr<osg::Light> dalishSun = new osg::Light;
                        dalishSun->setLightNum(6);
                        dalishSun->setPosition(osg::Vec4f(0.665468f, 0.123821f, -0.736085f, 0.f));
                        dalishSun->setAmbient(osg::Vec4f(0.f, 0.f, 0.f, 1.f));
                        dalishSun->setDiffuse(osg::Vec4f(0.7161f, 0.5099f, 0.3579f, 1.f));
                        dalishSun->setSpecular(osg::Vec4f(0.7161f, 0.5099f, 0.3579f, 1.f));
                        osg::ref_ptr<osg::LightSource> dalishSunSource = new osg::LightSource;
                        dalishSunSource->setLight(dalishSun);
                        dalishSunSource->setLocalStateSetModes(osg::StateAttribute::ON);
                        dalishSunSource->addChild(litNode);
                        litNode = dalishSunSource;
                    }
                    if (proofEnvEnabled("OPENMW_WORLD_VIEWER_EXTRA_SCENE_DAO_ATMOSPHERE")
                        && proofEnvEnabled("OPENMW_WORLD_VIEWER_EXTRA_SCENE_DAO_POINT_LIGHTS"))
                    {
                        struct DaoPointLight
                        {
                            osg::Vec3f mPosition;
                            osg::Vec3f mColour;
                            float mRadius;
                        };
                        // Six closest authored lak100d hotspots to the square.
                        const std::array<DaoPointLight, 6> points = { {
                            { { 244.134598f, 294.578247f, 22.259279f }, { 0.8f, 0.65f, 0.41f }, 15.f },
                            { { 244.134598f, 291.813202f, 29.117006f }, { 0.8f, 0.65f, 0.41f }, 15.f },
                            { { 280.929047f, 300.007904f, 16.98877f }, { 0.8f, 0.65f, 0.41f }, 15.f },
                            { { 262.021912f, 275.914795f, 14.883759f }, { 0.64f, 0.52f, 0.328f }, 15.f },
                            { { 238.969467f, 324.810364f, 35.426311f }, { 0.8f, 0.65f, 0.41f }, 20.f },
                            { { 282.905579f, 277.960205f, 14.884018f }, { 0.8f, 0.65f, 0.41f }, 10.f },
                        } };
                        for (std::size_t index = 0; index < points.size(); ++index)
                        {
                            const DaoPointLight& point = points[index];
                            osg::ref_ptr<osg::Light> pointLight = new osg::Light;
                            pointLight->setLightNum(static_cast<int>(index));
                            pointLight->setPosition(osg::Vec4f(
                                point.mPosition.x(), point.mPosition.y(), point.mPosition.z(), 1.f));
                            pointLight->setAmbient(osg::Vec4f(0.f, 0.f, 0.f, 1.f));
                            pointLight->setDiffuse(osg::Vec4f(
                                point.mColour.x(), point.mColour.y(), point.mColour.z(), 1.f));
                            pointLight->setSpecular(osg::Vec4f(
                                point.mColour.x() * 0.18f, point.mColour.y() * 0.18f,
                                point.mColour.z() * 0.18f, 1.f));
                            pointLight->setConstantAttenuation(1.f);
                            pointLight->setLinearAttenuation(1.f / point.mRadius);
                            osg::ref_ptr<osg::LightSource> source = new osg::LightSource;
                            source->setLight(pointLight);
                            source->setLocalStateSetModes(osg::StateAttribute::ON);
                            source->addChild(litNode);
                            litNode = source;
                        }
                    }
                    proofExtraScene->removeChild(node);
                    lightSource->addChild(litNode);
                    proofExtraScene->addChild(lightSource);
                }
                // osgDB scenes bypass Resource::SceneManager's normal shader
                // traversal. Apply it only to DAO face geodes: raw OBJ stages
                // do not name their generic texture units, so processing the
                // whole environment would invalidate otherwise-good materials.
                if (proofEnvEnabled("OPENMW_DAO_FACE_SHADER"))
                {
                    // The bundled glTF reader emits Geometry nodes directly
                    // beneath named mesh transforms. ShaderVisitor can process
                    // those in place once GLTFReader preserves the mesh name;
                    // OPENMW_DAO_FACE_ONLY prevents robe/hair replacement.
                    mResourceSystem->getSceneManager()->recreateShaders(proofExtraScene, "objects");
                    class DaoFaceGeodeCollector final : public osg::NodeVisitor
                    {
                    public:
                        DaoFaceGeodeCollector()
                            : osg::NodeVisitor(osg::NodeVisitor::TRAVERSE_ALL_CHILDREN)
                        {
                        }
                        ...
                        void apply(osg::Geode& geode) override
                        {
                            bool faceByPath = false;
                            for (osg::Node* node : getNodePath())
                            {
                                if (node == nullptr)
                                    continue;
                                const std::string& nodeName = node->getName();
                                if (nodeName.find("UHM") != std::string::npos
                            ...
                            for (unsigned int index = 0; index < geode.getNumDrawables(); ++index)
                            {
                                osg::StateSet* stateSet = geode.getDrawable(index)->getStateSet();
                                const auto* material = stateSet != nullptr
                                    ? dynamic_cast<const osg::Material*>(
                                          stateSet->getAttribute(osg::StateAttribute::MATERIAL))
                                    : nullptr;
                                const std::string name = material != nullptr ? material->getName() : std::string();
                                if (faceByPath || name.find("uh_hed") != std::string::npos)
                                    mDrawables.push_back({ &geode, geode.getDrawable(index) });
                            }
                            traverse(geode);
                        }
                        ...
                        struct FaceDrawable
                        {
                            osg::ref_ptr<osg::Geode> mSource;
                            osg::ref_ptr<osg::Drawable> mDrawable;
                        };
                        std::vector<FaceDrawable> mDrawables;
                    } collector;
                    proofExtraScene->accept(collector);
                    for (const DaoFaceGeodeCollector::FaceDrawable& face : collector.mDrawables)
                    {
                        osg::StateSet* stateSet = face.mDrawable->getOrCreateStateSet();
                        auto* material = dynamic_cast<osg::Material*>(
                            stateSet->getAttribute(osg::StateAttribute::MATERIAL));
                        if (material == nullptr)
                        {
                        ...
                        material->setName("uh_hed_fema");
                        mResourceSystem->getSceneManager()->recreateShaders(face.mSource, "objects");
                    }
                    Log(Debug::Info) << "DAO face shader traversal: drawables=" << collector.mDrawables.size();
                }
                SceneUtil::LightManager* lightRoot = mWorld->getRenderingManager()->getLightRoot();
                if (proofEnvEnabled("OPENMW_WORLD_VIEWER_EXTRA_SCENE_ISOLATE"))
                {
                    unsigned hiddenHostChildren = 0;
                    for (unsigned index = 0; index < lightRoot->getNumChildren(); ++index)
                    {
                        osg::Node* child = lightRoot->getChild(index);
                        if (child != nullptr && child->getNodeMask() != 0u)
                        {
                            child->setNodeMask(0u);
                            ++hiddenHostChildren;
                        }
                    }
                    Log(Debug::Info) << "World viewer extra scene: isolated imported root hiddenHostChildren="
                                     << hiddenHostChildren;
                }
                if (proofEnvEnabled("OPENMW_WORLD_VIEWER_EXTRA_SCENE_DAO_PBR") && !sharedMainCamera)
                {
                    osg::Group* sceneRoot
                        = mViewer->getSceneData() != nullptr ? mViewer->getSceneData()->asGroup() : nullptr;
                    if (proofExtraOuterScene.valid())
                    {
                        proofExtraOuterSceneCamera = new osg::Camera;
                        proofExtraOuterSceneCamera->setName("OpenDAO connected terrain camera");
                        proofExtraOuterSceneCamera->setReferenceFrame(osg::Transform::ABSOLUTE_RF);
                        proofExtraOuterSceneCamera->setProjectionMatrix(mViewer->getCamera()->getProjectionMatrix());
                        proofExtraOuterSceneCamera->setViewMatrix(mViewer->getCamera()->getViewMatrix());
                        proofExtraOuterSceneCamera->setViewport(mViewer->getCamera()->getViewport());
                        proofExtraOuterSceneCamera->setRenderOrder(osg::Camera::POST_RENDER, 40);
                        proofExtraOuterSceneCamera->setRenderTargetImplementation(osg::Camera::FRAME_BUFFER);
                        proofExtraOuterSceneCamera->setClearMask(GL_DEPTH_BUFFER_BIT);
                        SceneUtil::setCameraClearDepth(proofExtraOuterSceneCamera);
                        proofExtraOuterSceneCamera->setAllowEventFocus(false);
                        proofExtraOuterSceneCamera->addChild(proofExtraOuterScene);
                        if (sceneRoot != nullptr)
                            sceneRoot->addChild(proofExtraOuterSceneCamera);
                    }
                    proofExtraSceneCamera = new osg::Camera;
                    proofExtraSceneCamera->setName("OpenDAO isolated scene depth camera");
                    proofExtraSceneCamera->setReferenceFrame(osg::Transform::ABSOLUTE_RF);
                    proofExtraSceneCamera->setProjectionMatrix(mViewer->getCamera()->getProjectionMatrix());
                    proofExtraSceneCamera->setViewMatrix(mViewer->getCamera()->getViewMatrix());
                    proofExtraSceneCamera->setViewport(mViewer->getCamera()->getViewport());
                    proofExtraSceneCamera->setRenderOrder(osg::Camera::POST_RENDER, 50);
                    proofExtraSceneCamera->setRenderTargetImplementation(osg::Camera::FRAME_BUFFER);
                    proofExtraSceneCamera->setClearMask(GL_DEPTH_BUFFER_BIT);
                    SceneUtil::setCameraClearDepth(proofExtraSceneCamera);
                    proofExtraSceneCamera->setAllowEventFocus(false);
                    proofExtraSceneCamera->setNodeMask(~0u);
                    proofExtraSceneCamera->setCullMask(~0u);
                    proofExtraSceneCamera->addChild(proofExtraScene);
                    if (sceneRoot != nullptr)
                        sceneRoot->addChild(proofExtraSceneCamera);
                }
                else if (sharedMainCamera && mWorld->getPostProcessor() != nullptr)
                {
                    osg::Group* postSceneRoot = mWorld->getPostProcessor()->getSceneRoot();
                    postSceneRoot->addChild(proofExtraScene);

                    // OpenMW snapshots the opaque depth buffer from the
                    // DepthSortedBin draw callback. An isolated DAO portrait
                    // can contain only OPAQUE/MASK materials, so that bin has
                    // no render leaves and the snapshot never occurs. The
                    // postprocessor then sees cleared depth for every pixel,
                    // making physical depth of field impossible. Submit one
                    // invisible point in the transparent bin to guarantee the
                    // callback runs after opaque geometry. The point is fully
                    // transparent and depth-disabled; it changes no pixels.
                    osg::ref_ptr<osg::Geometry> depthCopyTrigger = new osg::Geometry;
                    osg::ref_ptr<osg::Vec3Array> triggerVertices = new osg::Vec3Array;
                    triggerVertices->push_back(osg::Vec3f(0.f, 0.f, 0.f));
                    depthCopyTrigger->setVertexArray(triggerVertices);
                    osg::ref_ptr<osg::Vec4Array> triggerColors = new osg::Vec4Array;
                    triggerColors->push_back(osg::Vec4f(0.f, 0.f, 0.f, 0.f));
                    depthCopyTrigger->setColorArray(triggerColors, osg::Array::BIND_OVERALL);
                    depthCopyTrigger->addPrimitiveSet(
                        new osg::DrawArrays(osg::PrimitiveSet::POINTS, 0, 1));
                    depthCopyTrigger->setCullingActive(false);

                    osg::StateSet* triggerState = depthCopyTrigger->getOrCreateStateSet();
                    triggerState->setMode(GL_BLEND, osg::StateAttribute::ON);
                    triggerState->setAttributeAndModes(new osg::BlendFunc(
                        osg::BlendFunc::SRC_ALPHA, osg::BlendFunc::ONE_MINUS_SRC_ALPHA));
                    triggerState->setAttributeAndModes(
                        new SceneUtil::AutoDepth(osg::Depth::ALWAYS, 0.0, 1.0, false));
                    triggerState->setRenderingHint(osg::StateSet::TRANSPARENT_BIN);
                    triggerState->setRenderBinDetails(10, "DepthSortedBin");

                    osg::ref_ptr<osg::Geode> triggerGeode = new osg::Geode;
                    triggerGeode->setName("OpenDAO opaque depth copy trigger");
                    triggerGeode->setCullingActive(false);
                    triggerGeode->addDrawable(depthCopyTrigger);
                    postSceneRoot->addChild(triggerGeode);
                }
                else
                    lightRoot->addChild(proofExtraScene);
                if (sharedMainCamera && proofEnvEnabled("OPENMW_PROOF_CAPTURE_POSTPROCESS"))
                {
                    MWRender::PostProcessor* postProcessor
                        = mWorld->getRenderingManager()->getPostProcessor();
                    postProcessor->enable();
                    postProcessor->triggerShaderReload();
                    Log(Debug::Info) << "OpenDAO portrait postprocess: forced enabled for shared main camera";
                }

                osg::ComputeBoundsVisitor boundsVisitor;
                proofExtraScene->accept(boundsVisitor);
                const osg::BoundingBox bounds = boundsVisitor.getBoundingBox();
                Log(Debug::Info) << "World viewer extra scene: loaded path=" << extraSceneSpec
    ...
    static osg::ref_ptr<osg::MatrixTransform> proofExtraForegroundTransform;
    if (proofWorldReady && mViewer != nullptr && !proofExtraForegroundSceneAttempted)
    {
        ...
        if (foregroundScenePath != nullptr && *foregroundScenePath != '\0')
        {
            ...
            else
            {
                ...
                proofExtraForegroundTransform = new osg::MatrixTransform;
                proofExtraForegroundTransform->setName("OpenMW local modern-asset proof foreground scene");
                proofExtraForegroundTransform->setMatrix(osg::Matrixf::scale(scale, scale, scale)
                    * osg::Matrixf::rotate(rotation) * osg::Matrixf::translate(offset));
                proofExtraForegroundTransform->addChild(foregroundNode);

                osg::StateSet* stateSet = proofExtraForegroundTransform->getOrCreateStateSet();
                if (!proofEnvEnabled("OPENMW_WORLD_VIEWER_EXTRA_SCENE_DAO_PBR"))
                    if (!proofEnvEnabled("OPENMW_WORLD_VIEWER_EXTRA_SCENE_DAO_PBR"))
                        stateSet->setAttributeAndModes(
                            new osg::Program, osg::StateAttribute::OFF | osg::StateAttribute::OVERRIDE);
                ...
                forceProofForegroundState(*proofExtraForegroundTransform);
                ...
                if (foregroundLight)
                {
                    osg::ref_ptr<osg::Light> light = new osg::Light;
                    light->setLightNum(7);
                    // Key the imported DAO actors from the same side as the
                    // locked square camera. The old negative-Y light was
                    // behind their authored headings and turned valid face
                    // albedo into featureless black silhouettes.
                    light->setPosition(osg::Vec4f(-0.55f, 0.65f, 0.75f, 0.f));
                    light->setAmbient(osg::Vec4f(0.22f, 0.21f, 0.19f, 1.f));
                    light->setDiffuse(osg::Vec4f(0.72f, 0.66f, 0.55f, 1.f));
                    light->setSpecular(osg::Vec4f(0.08f, 0.075f, 0.065f, 1.f));
                    osg::ref_ptr<osg::LightSource> lightSource = new osg::LightSource;
                    lightSource->setName("OpenMW local modern-asset proof foreground light");
                    lightSource->setLight(light);
                    lightSource->setLocalStateSetModes(osg::StateAttribute::ON);
                    lightSource->addChild(proofExtraForegroundTransform);
                    proofExtraForegroundCamera->addChild(lightSource);
                }
                else
                    proofExtraForegroundCamera->addChild(proofExtraForegroundTransform);
                if (osg::Group* sceneRoot
                    = mViewer->getSceneData() != nullptr ? mViewer->getSceneData()->asGroup() : nullptr)
                    sceneRoot->addChild(proofExtraForegroundCamera);

                osg::ComputeBoundsVisitor boundsVisitor;
                proofExtraForegroundTransform->accept(boundsVisitor);
                const osg::BoundingBox bounds = boundsVisitor.getBoundingBox();
    ...
    {
        const osg::Vec4f outdoorClear(
            readProofFloat("OPENMW_WORLD_VIEWER_PROOF_CLEAR_R", 0.38f),
            readProofFloat("OPENMW_WORLD_VIEWER_PROOF_CLEAR_G", 0.58f),
            readProofFloat("OPENMW_WORLD_VIEWER_PROOF_CLEAR_B", 0.82f), 1.f);
    ...
    if (proofWorldReady && mWorld != nullptr && !proofActorStaticCameraOwnsView)
    {
        ...
        // OpenMW's gameplay camera converts positions through Morrowind's
        // 64-units-per-metre convention. DAO glTF scenes are authored in
        // metres already, so that conversion put the real OSG eye kilometres
        // from Redcliffe even though the gameplay-camera audit looked valid.
        // For an isolated modern-asset proof, apply the authored keyframe to
        // the actual render camera after the gameplay camera has settled.
        if (proofEnvEnabled("OPENMW_WORLD_VIEWER_EXTRA_SCENE_DIRECT_CAMERA")
            && mViewer != nullptr && mViewer->getCamera() != nullptr)
        {
            static const std::vector<WorldViewerCameraKeyframe> directCameraSequence
                = getWorldViewerCameraSequence();
            int directCameraIndex = -1;
            for (std::size_t i = 0; i < directCameraSequence.size(); ++i)
            {
                if (frameNumber >= static_cast<unsigned>(directCameraSequence[i].mFrame))
                    directCameraIndex = static_cast<int>(i);
            }
            if (directCameraIndex >= 0)
            {
                const WorldViewerCameraKeyframe& keyframe
                    = directCameraSequence[static_cast<std::size_t>(directCameraIndex)];
                osg::Vec3d resolvedEye = keyframe.mEye;
                osg::Vec3d resolvedTarget = keyframe.mTarget;
                if (proofEnvEnabled("OPENMW_WORLD_VIEWER_DAO_RAYCAST_CAMERA")
                    && proofExtraScene.valid())
                {
                    const float waterHeight = readProofFloat(
                        "OPENMW_WORLD_VIEWER_DAO_NATIVE_WATER_HEIGHT", 0.f);
                    osg::ref_ptr<osgUtil::LineSegmentIntersector> down
                        = new osgUtil::LineSegmentIntersector(
                            osg::Vec3d(resolvedEye.x(), resolvedEye.y(), 1000.0),
                            osg::Vec3d(resolvedEye.x(), resolvedEye.y(), -1000.0));
                    osgUtil::IntersectionVisitor downVisitor(down);
                    proofExtraScene->accept(downVisitor);
                    // Imported DAO scenes also contain high, horizontal art
                    // cards (cloud/atmosphere and distant set dressing).  The
                    // first hit from above is therefore not necessarily the
                    // walkable surface.  Select the lowest plausible shoreline
                    // hit near the water datum and reject high decorative art.
                    double surface = static_cast<double>(waterHeight);
                    bool foundWalkableSurface = false;
                    if (down->containsIntersections())
                    {
                        for (const auto& intersection : down->getIntersections())
                        {
                            const double candidate = intersection.getWorldIntersectPoint().z();
                            if (candidate < static_cast<double>(waterHeight) - 10.0
                                || candidate > static_cast<double>(waterHeight) + 50.0)
                                continue;
                            if (!foundWalkableSurface || candidate < surface)
                            {
                                surface = candidate;
                    ...
                    resolvedEye.z() = std::max(
                        static_cast<double>(waterHeight + 2.2f), surface + 1.65);

                    // Lift only when scene geometry blocks the sight line well
                    // before the target. This is deterministic validation, not
                    // a camera offset guess.
                    for (int clearancePass = 0; clearancePass < 8; ++clearancePass)
                    {
                        osg::ref_ptr<osgUtil::LineSegmentIntersector> sight
                            = new osgUtil::LineSegmentIntersector(resolvedEye, resolvedTarget);
                        osgUtil::IntersectionVisitor sightVisitor(sight);
                        proofExtraScene->accept(sightVisitor);
                        if (!sight->containsIntersections()
                            || sight->getFirstIntersection().ratio > 0.90)
                            break;
                        resolvedEye.z() += 1.25;
                    }
                }
                mViewer->getCamera()->setViewMatrixAsLookAt(
                    resolvedEye, resolvedTarget, osg::Vec3d(0.0, 0.0, 1.0));
                const float directFov = readProofFloat(
                    "OPENMW_WORLD_VIEWER_EXTRA_SCENE_DIRECT_CAMERA_FOV_Y", 0.f);
                if (directFov > 1.f && directFov < 179.f)
                {
                    const osg::Viewport* viewport = mViewer->getCamera()->getViewport();
                    const double aspect = viewport && viewport->height() > 0.0
                        ? viewport->width() / viewport->height()
                        : (16.0 / 9.0);
                    if (SceneUtil::AutoDepth::isReversed())
                    {
                        mViewer->getCamera()->setProjectionMatrix(
                            SceneUtil::getReversedZProjectionMatrixAsPerspectiveInf(
                                directFov, aspect, 0.05));
                    }
                    else
                    {
                        mViewer->getCamera()->setProjectionMatrixAsPerspective(
                            directFov, aspect, 0.05, 4000.0);
                    }
                }
                static bool directCameraLogged = false;
                if (!directCameraLogged)
                {
                    directCameraLogged = true;
                    Log(Debug::Info) << "OPENDAO direct render camera eye=(" << resolvedEye.x() << ','
                                     << resolvedEye.y() << ',' << resolvedEye.z() << ") target=("
                                     << resolvedTarget.x() << ',' << resolvedTarget.y() << ','
                                     << resolvedTarget.z() << ") raycast="
                                     << proofEnvEnabled("OPENMW_WORLD_VIEWER_DAO_RAYCAST_CAMERA");
                }
            }
        }
    }

    // DAO exterior proofs use OpenMW's real water renderer. The imported
    // scene contributes shoreline geometry only; reflection, refraction,
    // animated normals, depth and surface timing remain native OpenMW.
    static bool proofDaoNativeWaterApplied = false;
    const char* proofDaoNativeWaterHeight = std::getenv("OPENMW_WORLD_VIEWER_DAO_NATIVE_WATER_HEIGHT");
    if (!proofDaoNativeWaterApplied && proofWorldReady && mWorld != nullptr
        && proofDaoNativeWaterHeight != nullptr && *proofDaoNativeWaterHeight != '\0')
    {
        const float waterHeight = readProofFloat("OPENMW_WORLD_VIEWER_DAO_NATIVE_WATER_HEIGHT", 0.f);
        // The empty proof cell does not declare water, so changing its height
        // alone leaves the renderer disabled.  Explicitly enable OpenMW's
        // native water system before selecting its height/toggle state.
        mWorld->getRenderingManager()->setWaterEnabled(true);
        mWorld->setWaterHeight(waterHeight);
        bool enabled = mWorld->toggleWater();
        if (!enabled)
            enabled = mWorld->toggleWater();
        proofDaoNativeWaterApplied = true;
        Log(Debug::Info) << "OPENDAO native OpenMW water height=" << waterHeight
                         << " enabled=" << enabled;
    }
    ...
    // The foreground camera is created while the loading camera is still
    // active. Keep it synchronized after the proof camera settles so actor
    // layers use the exact same eye, target, projection, and viewport.
    if (proofExtraForegroundCamera.valid() && mViewer != nullptr && mViewer->getCamera() != nullptr)
    {
        proofExtraForegroundCamera->setProjectionMatrix(mViewer->getCamera()->getProjectionMatrix());
        proofExtraForegroundCamera->setViewMatrix(mViewer->getCamera()->getViewMatrix());
        proofExtraForegroundCamera->setViewport(mViewer->getCamera()->getViewport());
    }
    if (proofExtraSceneCamera.valid() && mViewer != nullptr && mViewer->getCamera() != nullptr)
    {
        proofExtraSceneCamera->setProjectionMatrix(mViewer->getCamera()->getProjectionMatrix());
        proofExtraSceneCamera->setViewMatrix(mViewer->getCamera()->getViewMatrix());
        proofExtraSceneCamera->setViewport(mViewer->getCamera()->getViewport());
    }
    if (proofExtraOuterSceneCamera.valid() && mViewer != nullptr && mViewer->getCamera() != nullptr)
    {
        proofExtraOuterSceneCamera->setProjectionMatrix(mViewer->getCamera()->getProjectionMatrix());
        proofExtraOuterSceneCamera->setViewMatrix(mViewer->getCamera()->getViewMatrix());
        proofExtraOuterSceneCamera->setViewport(mViewer->getCamera()->getViewport());
    ...
    // Capture DAO state only after the static survey camera has been applied.
    // Writing it when the GLB load gate opened recorded the unrelated loading
    // camera and made numerical Godot/OpenMW comparisons misleading.
    static bool proofDaoTelemetryWritten = false;
    const char* proofDaoTelemetryPath = std::getenv("OPENMW_WORLD_VIEWER_DAO_TELEMETRY_PATH");
    const char* proofForegroundPath = std::getenv("OPENMW_WORLD_VIEWER_EXTRA_FOREGROUND_SCENE");
    const bool proofForegroundRequired = proofForegroundPath != nullptr && *proofForegroundPath != '\0';
    if (!proofDaoTelemetryWritten && proofDaoTelemetryPath != nullptr && *proofDaoTelemetryPath != '\0'
        && proofExtraScene != nullptr && (!proofForegroundRequired || proofExtraForegroundTransform != nullptr)
        && proofWorldReady && mViewer != nullptr && mViewer->getCamera() != nullptr)
    {
        const char* proofScenePaths = std::getenv("OPENMW_WORLD_VIEWER_EXTRA_SCENE");
        proofDaoTelemetryWritten = writeDaoRuntimeTelemetry(proofDaoTelemetryPath, proofExtraScene,
            proofExtraForegroundTransform, mViewer->getCamera(),
            proofScenePaths != nullptr ? proofScenePaths : "", proofForegroundPath != nullptr ? proofForegroundPath : "");
        Log(proofDaoTelemetryWritten ? Debug::Info : Debug::Error)
            << "OPENDAO runtime telemetry path=" << proofDaoTelemetryPath
            << " written=" << proofDaoTelemetryWritten;
    }
    ...
    if (!worldViewerNonStaticStartCameraSettled && proofWorldReady && proofWorldReadyFrames >= 2
    ...
    if (!fnvFlatStartupCameraSettled && proofWorldReady && proofWorldReadyFrames >= 2
    ...
    if (requestedCameraAngleSequenceIndex >= 0 && proofWorldReady && mWorld != nullptr)
    {
            ...
            || (proofSayFrame >= 0 && frameNumber >= static_cast<unsigned>(proofSayFrame))))
    {
        ...
        if (proofSayActor != nullptr && *proofSayActor != '\0')
        {
                ...
                && !proofNeutralActorPreviewIsolationApplied)
            {
                ...
                {
                    // Live Godot Dalish-camp background samples. OSG's clear
                    // value reaches this proof target without shader gamma.
                    mViewer->getCamera()->setClearColor(osg::Vec4(0.26f, 0.30f, 0.32f, 1.f));
    ...
    const bool actorMeshExportOnly = proofEnvEnabled("OPENMW_OPENNV_ACTOR_EXPORT_NO_SCREENSHOT")
        && std::getenv("OPENMW_OPENNV_ACTOR_EXPORT_ROOT") != nullptr
        && !proofActorBatchPrevious.isEmpty();
        ...
        && mScreenCaptureHandler != nullptr)
    {
        ...
        if (!actorMeshExportOnly && proofRequiresActorForScreenshot && !proofActorCameraAligned)
        {
        ...
        if (!actorMeshExportOnly && !worldViewerCameraReadyForScreenshot)
        {
        ...
        if (!actorMeshExportOnly && !proofActorVisualReady)
        {
        ...
        if (!actorMeshExportOnly && !proofActorPoseReadyForScreenshot)
        {
        ...
        if (!actorMeshExportOnly && !proofPortraitClear)
        {
        ...
        const bool actorMeshExported = !proofActorBatchPrevious.isEmpty()
            && exportOpenNvActorMesh(proofActorBatchPrevious, proofActorBatchIndex);
        if (actorMeshExportOnly)
        {
            Log(actorMeshExported ? Debug::Info : Debug::Error)
                << "OpenNV actor mesh export-only: index=" << proofActorBatchIndex
                << " status=" << (actorMeshExported ? "pass" : "fail")
                << " nativeScreenshot=0";
            if (proofScreenshotFrameReached)
                ++proofScreenshotFrameIndex;
            if (proofScreenshotFrameIndex >= proofActorBatchTargets.size()
                && proofEnvEnabled("OPENMW_OPENNV_ACTOR_EXPORT_EXIT_AFTER_BATCH"))
            {
                Log(Debug::Info) << "OpenNV actor mesh export-only: batch complete count="
                                 << proofScreenshotFrameIndex << "; exiting cleanly";
                mStateManager->requestQuit();
            }
            worldViewerTrace(frameNumber, "screenshot-queue.export-only.end");
        }
        else
        {
        if (proofPortraitClearRequired)
        {
            ...
            Log(Debug::Info) << "FNV/ESM4 proof: queuing GUI-inclusive native screenshot at frame " << frameNumber
                             ...
                             << " weatherTransition=" << mWorld->getWeatherTransition();
            if (proofEnvEnabled("OPENMW_PROOF_CAPTURE_POSTPROCESS") && mWorld != nullptr)
            {
                MWRender::PostProcessor* postProcessor = mWorld->getPostProcessor();
                Log(Debug::Info) << "OpenDAO portrait postprocess capture: enabled="
                                 << (postProcessor->isEnabled() ? 1 : 0)
                                 << " chain=" << postProcessor->getChain().size();
                static osg::ref_ptr<ProofPostProcessCaptureCallback> postProcessCapture;
                if (std::getenv("OPENMW_PROOF_POSTPROCESS_SCREENSHOT_PATH") == nullptr
                    && postProcessCapture == nullptr)
                {
                    osg::GraphicsContext* graphicsContext = mViewer->getCamera()->getGraphicsContext();
                    postProcessCapture = new ProofPostProcessCaptureCallback(
                        mScreenCaptureOperation, graphicsContext->getSwapCallback());
                    graphicsContext->setSwapCallback(postProcessCapture);
                }
                if (postProcessCapture != nullptr)
                    postProcessCapture->arm();
            }
            else
            {
                mScreenCaptureHandler->setFramesToCapture(1);
...
void OMW::Engine::prepareEngine()
{
    ...
    mStereoManager = std::make_unique<Stereo::Manager>(
        ...
        static_cast<int>(Settings::video().mAntialiasing));
