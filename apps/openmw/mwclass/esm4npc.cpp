...
namespace MWClass
{
    ...
    public:
        ...
        bool mFnvFurniturePackageHold = false;
    ...
    ESM4NpcCustomData::ESM4NpcCustomData(const ESM4NpcCustomData& other)
        ...
        , mFnvFurniturePackageHold(other.mFnvFurniturePackageHold)
    {
    }
    ...
    static int mapFnvAggressionToFight(std::uint8_t aggression)
    {
        switch (aggression)
        {
            case 0: // Unaggressive
    ...
    static int mapFnvConfidenceToFlee(std::uint8_t confidence)
    {
        switch (confidence)
        {
            case 0: // Cowardly
    ...
    static bool isEasyPeteProofActor(const ESM4::Npc& npc)
    {
        return Misc::StringUtils::ciEqual(npc.mEditorId, "GSEasyPete")
            || Misc::StringUtils::ciEqual(npc.mFullName, "Easy Pete");
    }
    ...
    static bool fnvPackageHasUnevaluatedConditions(const ESM4::AIPackage& package)
    {
    ...
    static const ESM4::AIPackage* selectFnvPackage(const std::vector<ESM::FormId>& packageIds, float hour)
    {
        ...
        for (ESM::FormId packageId : packageIds)
        {
            ...
            if (package == nullptr)
                continue;
            if (fnvPackageHasUnevaluatedConditions(*package))
                continue;
    ...
    static void stopFnvPackageMovement(MWWorld::Ptr& ptr)
    {
        ...
        ESM4NpcCustomData& data, const MWWorld::Ptr& ptr, const std::vector<ESM::FormId>& packageIds)
    {
        ...
        if (isFnvPackageTravelLike(package->mData.type))
        {
            ...
            if (furnitureTarget)
            {
                ...
                MWWorld::Ptr placed = world->moveObject(ptr, target->mPos.asVec3(), true, true);
                world->rotateObject(placed,
                    osg::Vec3f(target->mPos.rot[0], target->mPos.rot[1], target->mPos.rot[2]),
                    MWBase::RotationFlag_none);
                sequence.clear();
                data.mFnvFurniturePackageHold = true;
                stopFnvPackageMovement(placed);
                Log(Debug::Info) << "FNV/ESM4 diag: initialized native AI furniture package " << package->mEditorId
                                 << " type=" << getFnvPackageTypeName(package->mData.type) << " hour=" << hour
                                 << " override=" << usedHourOverride << " targetRef=" << target->mEditorId
                                 << " pos=(" << target->mPos.pos[0] << "," << target->mPos.pos[1] << ","
                                 << target->mPos.pos[2] << ") rot=(" << target->mPos.rot[0] << ","
                                 << target->mPos.rot[1] << "," << target->mPos.rot[2] << ") for "
                                 << traits->mEditorId << " movement=held";
                return;
            }
    ...
    static void initialiseActorStats(ESM4NpcCustomData& data)
    {
        ...
        const int fight = aiRecord->mIsFONV ? mapFnvAggressionToFight(aiRecord->mAIData.aggression)
                                            : aiRecord->mAIData.aggression;
        const int flee = aiRecord->mIsFONV ? mapFnvConfidenceToFlee(aiRecord->mAIData.confidence)
                                           : 100 - aiRecord->mAIData.confidence;
        ...
        if (aiRecord->mIsFONV && isEasyPeteProofActor(*aiRecord))
            Log(Debug::Info) << "FNV/ESM4 AI PROOF GSEasyPete: raw aggression="
                             << static_cast<int>(aiRecord->mAIData.aggression) << " confidence="
                             << static_cast<int>(aiRecord->mAIData.confidence) << " responsibility="
                             << static_cast<int>(aiRecord->mAIData.responsibility) << " mappedFight=" << fight
                             << " mappedFlee=" << flee << " mappedAlarm="
                             << static_cast<int>(aiRecord->mAIData.responsibility);
    ...
    void ESM4Npc::adjustPosition(const MWWorld::Ptr& ptr, bool force) const
    {
        const ESM4NpcCustomData& data = getCustomData(ptr);
        if (data.mTraits != nullptr && data.mTraits->mIsFONV)
        {
            const ESM::Position& pos = ptr.getRefData().getPosition();
            Log(Debug::Info) << "FNV/ESM4 diag: native actor ground adjustment for "
                             << data.mTraits->mEditorId << " pos=(" << pos.pos[0] << "," << pos.pos[1] << ","
                             << pos.pos[2] << ")";
        }

        Actor::adjustPosition(ptr, force);
    }

    osg::Vec3f ESM4Npc::getRotationVector(const MWWorld::Ptr& ptr) const
    {
        const ESM4NpcCustomData& data = getCustomData(ptr);
        if (!data.mFnvFurniturePackageHold)
            return Actor::getRotationVector(ptr);
