...
namespace MWClass
{
    ...
    static void initialiseActorStats(ESM4CreatureCustomData& data, const ESM4::Creature& creature)
    {
        ...
        stats.setAiSetting(MWMechanics::AiSetting::Fight,
            std::getenv("OPENMW_FNV_DISABLE_AI_PACKAGES") != nullptr ? 0
                                                                     : mapFnvAggressionToFight(creature.mAIData.aggression));
        stats.setAiSetting(MWMechanics::AiSetting::Flee, mapFnvConfidenceToFlee(creature.mAIData.confidence));
    ...
    void ESM4Creature::adjustPosition(const MWWorld::Ptr& ptr, bool force) const
    {
        const ESM4::Creature* base = ptr.get<ESM4::Creature>()->mBase;
        const ESM4::Creature& effective = getEffectiveCreature(*base);
        const ESM::Position& pos = ptr.getRefData().getPosition();
        Log(Debug::Info) << "FNV/ESM4 diag: native creature ground adjustment for "
                         << effective.mEditorId << " pos=(" << pos.pos[0] << "," << pos.pos[1] << ","
                         << pos.pos[2] << ")";
