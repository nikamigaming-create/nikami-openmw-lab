...
namespace MWWorld
{
        ...
        const FalloutPlayerState* nativePlayerState, std::span<const std::string> currentContentFiles)
    {
        ...
        {
            return loadFailure("resolved native FNV Player identity does not preserve the exact FalloutNV.esm "
        ...
        {
            return loadFailure(std::string("FNV save has no unique canonical Player ACHR change form: ") + error.what());
        }
