...
namespace MWWorld
{
        ...
        const Store<ESM4::Npc>& npcs, ESM::FormId normalizedPlayerFormId)
    {
        ...
        if (player == nullptr)
        {
            return failure("missing winning Fallout master Player NPC_ FormID 0x00000007 at normalized "
                + normalizedPlayerFormId.toString());
        }
        if (!player->mIsFONV)
            return failure("winning NPC_ FormID 0x00000007 is not a Fallout 3/New Vegas record");
