...
namespace MWWorld
{
    ...
    void WeatherManager::update(float duration, bool paused, const TimeStamp& time, bool isExterior)
    {
        ...
        const auto applyFalloutImageSpace = [&]() {
            ...
            if (base == nullptr)
            {
                // Do not carry a previous exterior's cinematic constants into an interior without XCIM.
                if (hasImageSpaceContext)
                {
                    mRendering.getPostProcessor()->setFalloutImageSpace(osg::Vec4f(1.f, 0.f, 1.f, 1.f),
                        osg::Vec4f(1.f, 0.f, 1.f, 1.f), osg::Vec4f(1.f, 1.f, 1.f, 0.f), osg::Vec4f());
                }
                return;
            }
        ...
        if (!isExterior)
        {
            // Interior CELL XCIM is independent of exterior weather. Apply it before the weather-only early return
            // so an exterior grade cannot leak into the newly loaded interior.
