...
namespace
{
    ...
    void ensureFalloutCharacterDefaults(MWWorld::Store<ESM::Class>& classes, MWWorld::Store<ESM::Race>& races,
        MWWorld::Store<ESM::Skill>& skills, MWWorld::Store<ESM::NPC>& npcs, MWWorld::Store<ESM::Weapon>& weapons,
...
namespace MWWorld
{
    ...
    void ESMStore::validate()
    {
        ...
        if (mESM4Game == ESM4Game::Fallout3 || mESM4Game == ESM4Game::FalloutNewVegas)
        {
            if (!mESM4GameMasterIndex)
                throw std::runtime_error(
                    "Cannot resolve native Fallout Player state: missing game-master load-order index");
            ...
            if (!resolution)
                throw std::runtime_error("Cannot resolve native Fallout Player state: " + resolution.mError);
            mFalloutPlayerState = std::move(resolution.mState);
            Log(Debug::Info) << "Fallout/ESM4: resolved native Player state base=" << mFalloutPlayerState->mBaseRecord
