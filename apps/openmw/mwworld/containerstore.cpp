...
namespace
{
    ...
    float getRecordWeight(const ESM4::Ammunition& record)
    {
        return record.mData.mWeight;
    }
    ...
    float getRecordWeight(const ESM4::Book& record)
    {
        return record.mData.weight;
    }

    float getRecordWeight(const ESM4::Clothing& record)
    {
        return record.mData.weight;
    }

    float getRecordWeight(const ESM4::Ingredient& record)
    {
        return record.mData.weight;
    }

    float getRecordWeight(const ESM4::ItemMod& record)
    {
        return record.mData.mWeight;
    }

    float getRecordWeight(const ESM4::Key& record)
    {
        return record.mData.weight;
    }

    float getRecordWeight(const ESM4::Light& record)
    {
        return record.mData.weight;
    }
...
MWWorld::ContainerStoreIterator MWWorld::ContainerStore::addNewStack(const ConstPtr& ptr, int count)
{
    ...
    switch (getType(ptr))
    {
        ...
        case Type_ESM4Ammunition:
            mLists.mESM4Ammunition.mList.push_back(*ptr.get<ESM4::Ammunition>());
            it = ContainerStoreIterator(this, --mLists.mESM4Ammunition.mList.end());
            break;
        case Type_ESM4Armor:
            mLists.mESM4Armors.mList.push_back(*ptr.get<ESM4::Armor>());
            it = ContainerStoreIterator(this, --mLists.mESM4Armors.mList.end());
            break;
        case Type_ESM4Miscellaneous:
            mLists.mESM4MiscItems.mList.push_back(*ptr.get<ESM4::MiscItem>());
            it = ContainerStoreIterator(this, --mLists.mESM4MiscItems.mList.end());
            break;
        case Type_ESM4Weapon:
            mLists.mESM4Weapons.mList.push_back(*ptr.get<ESM4::Weapon>());
            it = ContainerStoreIterator(this, --mLists.mESM4Weapons.mList.end());
            break;
        case Type_ESM4Potion:
            mLists.mESM4Potions.mList.push_back(*ptr.get<ESM4::Potion>());
            it = ContainerStoreIterator(this, --mLists.mESM4Potions.mList.end());
            break;
        case Type_ESM4Book:
            mLists.mESM4Books.mList.push_back(*ptr.get<ESM4::Book>());
            it = ContainerStoreIterator(this, --mLists.mESM4Books.mList.end());
            break;
        case Type_ESM4Clothing:
            mLists.mESM4Clothes.mList.push_back(*ptr.get<ESM4::Clothing>());
            it = ContainerStoreIterator(this, --mLists.mESM4Clothes.mList.end());
            break;
        case Type_ESM4Ingredient:
            mLists.mESM4Ingredients.mList.push_back(*ptr.get<ESM4::Ingredient>());
            it = ContainerStoreIterator(this, --mLists.mESM4Ingredients.mList.end());
            break;
        case Type_ESM4ItemMod:
            mLists.mESM4ItemMods.mList.push_back(*ptr.get<ESM4::ItemMod>());
            it = ContainerStoreIterator(this, --mLists.mESM4ItemMods.mList.end());
            break;
        case Type_ESM4Key:
            mLists.mESM4Keys.mList.push_back(*ptr.get<ESM4::Key>());
            it = ContainerStoreIterator(this, --mLists.mESM4Keys.mList.end());
            break;
        case Type_ESM4Light:
            mLists.mESM4Lights.mList.push_back(*ptr.get<ESM4::Light>());
            it = ContainerStoreIterator(this, --mLists.mESM4Lights.mList.end());
            break;
    }
...
float MWWorld::ContainerStore::getWeight() const
{
    if (!mWeightUpToDate)
    {
        ...
        mCachedWeight += getTotalWeight(mLists.mESM4Ammunition);
        mCachedWeight += getTotalWeight(mLists.mESM4Armors);
        mCachedWeight += getTotalWeight(mLists.mESM4MiscItems);
        mCachedWeight += getTotalWeight(mLists.mESM4Weapons);
        mCachedWeight += getTotalWeight(mLists.mESM4Potions);
        mCachedWeight += getTotalWeight(mLists.mESM4Books);
        mCachedWeight += getTotalWeight(mLists.mESM4Clothes);
        mCachedWeight += getTotalWeight(mLists.mESM4Ingredients);
        mCachedWeight += getTotalWeight(mLists.mESM4ItemMods);
        mCachedWeight += getTotalWeight(mLists.mESM4Keys);
        mCachedWeight += getTotalWeight(mLists.mESM4Lights);
...
int MWWorld::ContainerStore::getType(const ConstPtr& ptr)
{
    ...
    if (ptr.getType() == ESM4::Book::sRecordId)
        return Type_ESM4Book;

    if (ptr.getType() == ESM4::Clothing::sRecordId)
        return Type_ESM4Clothing;

    if (ptr.getType() == ESM4::Ingredient::sRecordId)
        return Type_ESM4Ingredient;

    if (ptr.getType() == ESM4::ItemMod::sRecordId)
        return Type_ESM4ItemMod;

    if (ptr.getType() == ESM4::Key::sRecordId)
        return Type_ESM4Key;

    if (ptr.getType() == ESM4::Light::sRecordId)
        return Type_ESM4Light;
...
MWWorld::Ptr MWWorld::ContainerStore::search(const ESM::RefId& id)
{
    ...
    {
        Ptr ptr = searchId(mLists.mESM4Ammunition, id, this);
        if (!ptr.isEmpty())
            return ptr;
    }

    {
        Ptr ptr = searchId(mLists.mESM4Armors, id, this);
        if (!ptr.isEmpty())
            return ptr;
    }

    {
        Ptr ptr = searchId(mLists.mESM4MiscItems, id, this);
        if (!ptr.isEmpty())
            return ptr;
    }

    {
        Ptr ptr = searchId(mLists.mESM4Weapons, id, this);
        if (!ptr.isEmpty())
            return ptr;
    }

    {
        Ptr ptr = searchId(mLists.mESM4Potions, id, this);
        if (!ptr.isEmpty())
            return ptr;
    }

    {
        Ptr ptr = searchId(mLists.mESM4Books, id, this);
        if (!ptr.isEmpty())
            return ptr;
    }

    {
        Ptr ptr = searchId(mLists.mESM4Clothes, id, this);
        if (!ptr.isEmpty())
            return ptr;
    }

    {
        Ptr ptr = searchId(mLists.mESM4Ingredients, id, this);
        if (!ptr.isEmpty())
            return ptr;
    }

    {
        Ptr ptr = searchId(mLists.mESM4ItemMods, id, this);
        if (!ptr.isEmpty())
            return ptr;
    }

    {
        Ptr ptr = searchId(mLists.mESM4Keys, id, this);
        if (!ptr.isEmpty())
            return ptr;
    }

    {
        Ptr ptr = searchId(mLists.mESM4Lights, id, this);
        if (!ptr.isEmpty())
            return ptr;
    }
...
void MWWorld::ContainerStore::writeState(ESM::InventoryState& state) const
{
    ...
    storeStates(mLists.mESM4Ammunition, state, index);
    storeStates(mLists.mESM4Armors, state, index, true);
    storeStates(mLists.mESM4MiscItems, state, index);
    storeStates(mLists.mESM4Weapons, state, index, true);
    storeStates(mLists.mESM4Potions, state, index);
    storeStates(mLists.mESM4Books, state, index, true);
    storeStates(mLists.mESM4Clothes, state, index, true);
    storeStates(mLists.mESM4Ingredients, state, index);
    storeStates(mLists.mESM4ItemMods, state, index);
    storeStates(mLists.mESM4Keys, state, index);
    storeStates(mLists.mESM4Lights, state, index, true);
}
    ...
    for (const ESM::ObjectState& state : inventory.mItems)
    {
        ...
        switch (type)
        {
            ...
            case ESM::REC_LIGH:
                ...
                break;
            case ESM::REC_AMMO4:
                getState(mLists.mESM4Ammunition, state);
                break;
            case ESM::REC_ARMO4:
                readEquipmentState(getState(mLists.mESM4Armors, state), thisIndex, inventory);
                break;
            case ESM::REC_MISC4:
                getState(mLists.mESM4MiscItems, state);
                break;
            case ESM::REC_WEAP4:
                readEquipmentState(getState(mLists.mESM4Weapons, state), thisIndex, inventory);
                break;
            case ESM::REC_ALCH4:
                getState(mLists.mESM4Potions, state);
                break;
            case ESM::REC_BOOK4:
                readEquipmentState(getState(mLists.mESM4Books, state), thisIndex, inventory);
                break;
            case ESM::REC_CLOT4:
                readEquipmentState(getState(mLists.mESM4Clothes, state), thisIndex, inventory);
                break;
            case ESM::REC_INGR4:
                getState(mLists.mESM4Ingredients, state);
                break;
            case ESM::REC_IMOD4:
                getState(mLists.mESM4ItemMods, state);
                break;
            case ESM::REC_KEYM4:
                getState(mLists.mESM4Keys, state);
                break;
            case ESM::REC_LIGH4:
                readEquipmentState(getState(mLists.mESM4Lights, state), thisIndex, inventory);
                break;
            case 0:
...
void MWWorld::ContainerStoreIteratorBase<PtrType>::copy(const ContainerStoreIteratorBase<T>& src)
{
    ...
    switch (src.mType)
    {
        ...
        case MWWorld::ContainerStore::Type_ESM4Potion:
            ...
            break;
        case MWWorld::ContainerStore::Type_ESM4Book:
            mESM4Book = src.mESM4Book;
            break;
        case MWWorld::ContainerStore::Type_ESM4Clothing:
            mESM4Clothing = src.mESM4Clothing;
            break;
        case MWWorld::ContainerStore::Type_ESM4Ingredient:
            mESM4Ingredient = src.mESM4Ingredient;
            break;
        case MWWorld::ContainerStore::Type_ESM4ItemMod:
            mESM4ItemMod = src.mESM4ItemMod;
            break;
        case MWWorld::ContainerStore::Type_ESM4Key:
            mESM4Key = src.mESM4Key;
            break;
        case MWWorld::ContainerStore::Type_ESM4Light:
            mESM4Light = src.mESM4Light;
            break;
        case -1:
            break;
        default:
...
bool MWWorld::ContainerStoreIteratorBase<PtrType>::resetIterator()
{
    switch (mType)
    {
        ...
        case ContainerStore::Type_ESM4Ammunition:
            ...
            mESM4Ammunition = mContainer->mLists.mESM4Ammunition.mList.begin();
            return mESM4Ammunition != mContainer->mLists.mESM4Ammunition.mList.end();

        case ContainerStore::Type_ESM4Armor:

            mESM4Armor = mContainer->mLists.mESM4Armors.mList.begin();
            return mESM4Armor != mContainer->mLists.mESM4Armors.mList.end();

        case ContainerStore::Type_ESM4Miscellaneous:

            mESM4Miscellaneous = mContainer->mLists.mESM4MiscItems.mList.begin();
            return mESM4Miscellaneous != mContainer->mLists.mESM4MiscItems.mList.end();

        case ContainerStore::Type_ESM4Weapon:

            mESM4Weapon = mContainer->mLists.mESM4Weapons.mList.begin();
            return mESM4Weapon != mContainer->mLists.mESM4Weapons.mList.end();

        case ContainerStore::Type_ESM4Potion:

            mESM4Potion = mContainer->mLists.mESM4Potions.mList.begin();
            return mESM4Potion != mContainer->mLists.mESM4Potions.mList.end();

        case ContainerStore::Type_ESM4Book:

            mESM4Book = mContainer->mLists.mESM4Books.mList.begin();
            return mESM4Book != mContainer->mLists.mESM4Books.mList.end();

        case ContainerStore::Type_ESM4Clothing:

            mESM4Clothing = mContainer->mLists.mESM4Clothes.mList.begin();
            return mESM4Clothing != mContainer->mLists.mESM4Clothes.mList.end();

        case ContainerStore::Type_ESM4Ingredient:

            mESM4Ingredient = mContainer->mLists.mESM4Ingredients.mList.begin();
            return mESM4Ingredient != mContainer->mLists.mESM4Ingredients.mList.end();

        case ContainerStore::Type_ESM4ItemMod:

            mESM4ItemMod = mContainer->mLists.mESM4ItemMods.mList.begin();
            return mESM4ItemMod != mContainer->mLists.mESM4ItemMods.mList.end();

        case ContainerStore::Type_ESM4Key:

            mESM4Key = mContainer->mLists.mESM4Keys.mList.begin();
            return mESM4Key != mContainer->mLists.mESM4Keys.mList.end();

        case ContainerStore::Type_ESM4Light:

            mESM4Light = mContainer->mLists.mESM4Lights.mList.begin();
            return mESM4Light != mContainer->mLists.mESM4Lights.mList.end();
    }
...
bool MWWorld::ContainerStoreIteratorBase<PtrType>::isEqual(const ContainerStoreIteratorBase<T>& other) const
{
    ...
    switch (mType)
    {
        ...
        case ContainerStore::Type_ESM4Book:
            return mESM4Book == other.mESM4Book;
        case ContainerStore::Type_ESM4Clothing:
            return mESM4Clothing == other.mESM4Clothing;
        case ContainerStore::Type_ESM4Ingredient:
            return mESM4Ingredient == other.mESM4Ingredient;
        case ContainerStore::Type_ESM4ItemMod:
            return mESM4ItemMod == other.mESM4ItemMod;
        case ContainerStore::Type_ESM4Key:
            return mESM4Key == other.mESM4Key;
        case ContainerStore::Type_ESM4Light:
            return mESM4Light == other.mESM4Light;
        case -1:
            return true;
    }
...
PtrType MWWorld::ContainerStoreIteratorBase<PtrType>::operator*() const
{
    ...
    switch (mType)
    {
        ...
        case ContainerStore::Type_ESM4Potion:
            ...
            break;
        case ContainerStore::Type_ESM4Book:
            ptr = PtrType(&*mESM4Book, nullptr);
            break;
        case ContainerStore::Type_ESM4Clothing:
            ptr = PtrType(&*mESM4Clothing, nullptr);
            break;
        case ContainerStore::Type_ESM4Ingredient:
            ptr = PtrType(&*mESM4Ingredient, nullptr);
            break;
        case ContainerStore::Type_ESM4ItemMod:
            ptr = PtrType(&*mESM4ItemMod, nullptr);
            break;
        case ContainerStore::Type_ESM4Key:
            ptr = PtrType(&*mESM4Key, nullptr);
            break;
        case ContainerStore::Type_ESM4Light:
            ptr = PtrType(&*mESM4Light, nullptr);
            break;
    }
...
template <class PtrType>
MWWorld::ContainerStoreIteratorBase<PtrType>::ContainerStoreIteratorBase(
    ContainerStoreType container, typename Iterator<ESM4::Book>::type iterator)
    : mType(MWWorld::ContainerStore::Type_ESM4Book)
    , mMask(MWWorld::ContainerStore::Type_All)
    , mContainer(container)
    , mESM4Book(iterator)
{
...
template <class PtrType>
MWWorld::ContainerStoreIteratorBase<PtrType>::ContainerStoreIteratorBase(
    ContainerStoreType container, typename Iterator<ESM4::Clothing>::type iterator)
    : mType(MWWorld::ContainerStore::Type_ESM4Clothing)
    , mMask(MWWorld::ContainerStore::Type_All)
    , mContainer(container)
    , mESM4Clothing(iterator)
{
}

template <class PtrType>
MWWorld::ContainerStoreIteratorBase<PtrType>::ContainerStoreIteratorBase(
    ContainerStoreType container, typename Iterator<ESM4::Ingredient>::type iterator)
    : mType(MWWorld::ContainerStore::Type_ESM4Ingredient)
    , mMask(MWWorld::ContainerStore::Type_All)
    , mContainer(container)
    , mESM4Ingredient(iterator)
{
}

template <class PtrType>
MWWorld::ContainerStoreIteratorBase<PtrType>::ContainerStoreIteratorBase(
    ContainerStoreType container, typename Iterator<ESM4::ItemMod>::type iterator)
    : mType(MWWorld::ContainerStore::Type_ESM4ItemMod)
    , mMask(MWWorld::ContainerStore::Type_All)
    , mContainer(container)
    , mESM4ItemMod(iterator)
{
}

template <class PtrType>
MWWorld::ContainerStoreIteratorBase<PtrType>::ContainerStoreIteratorBase(
    ContainerStoreType container, typename Iterator<ESM4::Key>::type iterator)
    : mType(MWWorld::ContainerStore::Type_ESM4Key)
    , mMask(MWWorld::ContainerStore::Type_All)
    , mContainer(container)
    , mESM4Key(iterator)
{
...
template <class PtrType>
MWWorld::ContainerStoreIteratorBase<PtrType>::ContainerStoreIteratorBase(
    ContainerStoreType container, typename Iterator<ESM4::Light>::type iterator)
    : mType(MWWorld::ContainerStore::Type_ESM4Light)
    , mMask(MWWorld::ContainerStore::Type_All)
    , mContainer(container)
    , mESM4Light(iterator)
{
