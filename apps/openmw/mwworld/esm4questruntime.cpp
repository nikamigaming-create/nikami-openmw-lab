...
namespace MWWorld
{
    void ESM4QuestRuntime::initialize(const ESMStore& store, const Globals* globals)
    {
        ...
        const auto registerReferenceScript = [this, &store, &resolveReferenceBase](const ESM4::Reference& reference) {
            ...
            if (!triggerEnter && !triggerLeave && !trigger && !activate)
                return;
            ...
            state.mHasTriggerEnter = triggerEnter;
            state.mHasTriggerLeave = triggerLeave;
            state.mHasTrigger = trigger;
            for (const ESM4::ScriptLocalVariableData& variable : script->mScript.localVarData)
                if (!variable.variableName.empty())
                    state.mVariables.emplace(Misc::StringUtils::lowerCase(variable.variableName), 0.f);
    ...
    void ESM4QuestRuntime::update(float duration, bool paused)
    {
        ...
        // declares which event blocks it handles, while its authored bounds
        // determine the volume.  This makes physical scene entry visible to
        // the same source runtime as a real activation, with no quest/cell
        // names or campaign route embedded in the engine.
        for (auto& [referenceId, state] : mReferenceScriptStates)
        {
            if (!state.mHasTriggerEnter && !state.mHasTriggerLeave && !state.mHasTrigger)
                continue;

            MWWorld::Ptr reference = world->searchPtrByRefNum(referenceId);
            if (reference.isEmpty() || reference.getCell() == nullptr || state.mCell != playerCellId)
            {
            ...
            const bool inside = (playerPosition - referencePosition).length2() <= radius * radius;
            const bool entered = inside && !state.mPlayerWasInside;
            const bool left = !inside && state.mPlayerWasInside;
            state.mPlayerWasInside = inside;

            if (entered && state.mHasTriggerEnter)
            {
                executeStageSource(state.mSource, {}, duration, {}, "ontriggerenter", {}, referenceId, "player");
                Log(Debug::Info) << "FNV/ESM4 behavior: reference-script event reference=" << state.mEditorId
                                 << " event=ontriggerenter argument=player radius=" << radius;
            }
            if (inside && state.mHasTrigger)
            {
                executeStageSource(state.mSource, {}, duration, {}, "ontrigger", {}, referenceId, "player");
            }
            if (left && state.mHasTriggerLeave)
            {
                executeStageSource(state.mSource, {}, duration, {}, "ontriggerleave", {}, referenceId, "player");
                Log(Debug::Info) << "FNV/ESM4 behavior: reference-script event reference=" << state.mEditorId
                                 << " event=ontriggerleave argument=player radius=" << radius;
            }
        }
        ...
        std::string_view actionReferenceArgument)
    {
        ...
        const auto sourceValue = [this, &sourceVariable, secondsPassed, &actionReference](const SourceTokens& tokens,
