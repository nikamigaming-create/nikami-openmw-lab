...
namespace MWGui
{
    ...
    void CharacterCreation::onFrame(float duration)
    {
        ...
        if (mAuthoredNameMenu && mNameDialog && mAuthoredNameMenuDefaultDelay >= 0.f)
        {
            ...
            {
                // Proof automation is opt-in.  Normal launches leave the
                // modal open until the player enters a name and confirms it.
        ...
        if (mAuthoredRaceMenu && mAuthoredAppearanceDialog && mAuthoredRaceMenuDefaultDelay >= 0.f)
        {
            ...
            {
                // Advance through the same dialog callback a player reaches
                // with its Done button. The delay is opt-in and exists only
                // for unattended proof runs, never for normal play.
        ...
        try
        {
            switch (id)
            {
                ...
                case GM_Race:
                    ...
                    {
                        // Fallout's NPC_ player record has no ESM3 race/body-part
                        // representation.  Routing this handoff through RaceDialog
                        // produced a blank preview and fallback Wastelander controls.
                        // Keep the interaction separate, use the current native
                        // player proxy as the data source, and let the normal player
                        // rebuild path apply the selected sex.
