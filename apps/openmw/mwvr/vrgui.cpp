...
namespace MWVR
{
    namespace
    {
        ...
        {
            return layerName == "HUD" || layerName == "HUD_3D";
        }
    ...
    {
    public:
        GUIRTT(int width, int height, osg::Vec4 clearColor, osg::ref_ptr<osg::Node> scene, std::string label)
            : RTTNode(width, height, 1, false, 0, StereoAwareness::Unaware, false)
            , mScene(scene)
        ...
        void setDefaults(osg::Camera* camera) override
        {
            ...
            setName("VRGUIRTT_" + mLabel);
            camera->setName("VRGUIRTT_Camera_" + mLabel);
    ...
    void VRGUILayer::updateVisibility()
    {
        ...
        {
            if (luaWidget && luaWidget->mRoot && luaWidget->mRoot->widget())
                visible = visible || luaWidget->mRoot->widget()->getVisible();
        }
    ...
    void VRGUILayer::updateRect()
    {
        ...
        for (const auto& element : mLuaElements)
        {
            if (!element || !element->mRoot)
                continue;
            if (!element->mRoot->widget() || !element->mRoot->widget()->getVisible())
                continue;
    ...
    void VRGUILayer::update(osg::NodeVisitor* nv)
    {
        ...
        if (mDirty)
        {
            ...
            auto* rttNode = new GUIRTT(mConfig->pixelResolution.x(), mConfig->pixelResolution.y(),
                osg::Vec4(0, 0, 0, mConfig->opacity), mMyGUICamera, mLayerName);
            mGUIRTT = rttNode;
        ...
        if (mVisible && isFalloutHudLayer(mLayerName) && sFalloutHudRectLogCount < 12)
        {
            ...
            Log(Debug::Info) << "FNV/ESM4 proof: wrist HUD layer rect layer=" << mLayerName
                             << " widgets=" << mWidgets.size()
    ...
    void VRGUIManager::readConfig()
    {
        ...
        LayerConfig hudConfig = createDefaultConfig(false, true);
        hudConfig.center = osg::Vec2(0.f, 0.5f);
        hudConfig.extent = osg::Vec2(0.16f, 0.065f);
        hudConfig.spatialResolution = 1300;
        hudConfig.space = "LeftWristTop";
        LayerConfig hud3DConfig = createDefaultConfig(false, true);
        hud3DConfig.opacity = 0.08f;
        hud3DConfig.center = osg::Vec2(0.f, 0.5f);
        hud3DConfig.extent = osg::Vec2(0.16f, 0.065f);
        hud3DConfig.spatialResolution = 1300;
        hud3DConfig.space = "LeftWristTop";
        LayerConfig pipBoyInventoryConfig = createDefaultConfig(false, false);
        pipBoyInventoryConfig.opacity = 0.08f;
        pipBoyInventoryConfig.extent = osg::Vec2(0.18f, 0.13f);
        pipBoyInventoryConfig.spatialResolution = 900;
        pipBoyInventoryConfig.pixelResolution = osg::Vec2i(1024, 768);
        pipBoyInventoryConfig.space = "PipBoyInventory";
        LayerConfig pipBoyStatsConfig = createDefaultConfig(false, false);
        pipBoyStatsConfig.opacity = 0.05f;
        pipBoyStatsConfig.extent = osg::Vec2(0.09f, 0.12f);
        pipBoyStatsConfig.spatialResolution = 900;
        pipBoyStatsConfig.pixelResolution = osg::Vec2i(768, 768);
        pipBoyStatsConfig.space = "PipBoyStats";
        LayerConfig pipBoyMapConfig = pipBoyStatsConfig;
        pipBoyMapConfig.space = "PipBoyMap";
        LayerConfig pipBoyUtilityConfig = createDefaultConfig(false, false);
        pipBoyUtilityConfig.opacity = 0.05f;
        pipBoyUtilityConfig.extent = osg::Vec2(0.12f, 0.05f);
        pipBoyUtilityConfig.spatialResolution = 900;
        pipBoyUtilityConfig.pixelResolution = osg::Vec2i(768, 384);
        pipBoyUtilityConfig.space = "PipBoyUtility";
        ...
        mDefaultLayerConfigs = {
            ...
            { "HUD", hudConfig },
            { "HUD_3D", hud3DConfig },
            { "InventoryWindow", pipBoyInventoryConfig },
            { "StatsWindow", pipBoyStatsConfig },
            { "MapWindow", pipBoyMapConfig },
            { "SpellWindow", pipBoyUtilityConfig },
            ...
            { "LoadingScreenBackground", loadingScreenConfig },
            { "LoadingScreen", loadingScreenConfig },
        };
        Log(Debug::Info) << "FNV/ESM4 proof: native VR loading screen quad defaults configured extent=("
                         << loadingScreenConfig.extent.x() << "," << loadingScreenConfig.extent.y() << ")"
                         << " rtt=(" << loadingScreenConfig.pixelResolution.x() << ","
                         << loadingScreenConfig.pixelResolution.y() << ")"
                         << " backgroundOpacity=" << loadingScreenConfig.opacity;
        Log(Debug::Info) << "FNV/ESM4 proof: native VR wrist HUD defaults configured space=" << hud3DConfig.space
                         << " extent=(" << hud3DConfig.extent.x() << "," << hud3DConfig.extent.y() << ")"
                         << " ppm=" << hud3DConfig.spatialResolution;
        Log(Debug::Info) << "FNV/ESM4 proof: native VR PipBoy layer defaults configured inventory="
                         << pipBoyInventoryConfig.space << " stats=" << pipBoyStatsConfig.space
                         << " map=" << pipBoyMapConfig.space << " utility=" << pipBoyUtilityConfig.space;
    ...
    bool VRGUIManager::updateAndroidOpenXRQuadFocus(const Stereo::Pose& pointerPose, float* distanceMeters)
    {
#if defined(__ANDROID__)
        std::scoped_lock lock(mMutex);

        VRGUILayer* bestLayer = nullptr;
        osg::Vec3 bestLocal;
        float bestU = 0.f;
        float bestV = 0.f;
        float bestDistance = std::numeric_limits<float>::max();

        const osg::Vec3 origin = pointerPose.position.asMeters();
        osg::Vec3 direction = pointerPose.orientation * osg::Vec3(0.f, 1.f, 0.f);
        if (direction.length2() <= 0.000001f)
            return false;
        direction.normalize();

        for (auto& item : mLayers)
        {
            VRGUILayer* layer = item.second.get();
            if (!layer || !layer->visible() || !layer->mConfig || !layer->mPickable)
                continue;

            Stereo::Pose quadPose = layer->mPose;
            if (layer->mSpace)
            {
                auto tracked = layer->mSpace->locateInWorld();
                if (!tracked.status)
                    continue;
                quadPose = tracked.pose + quadPose;
            }

            const osg::Vec3 center = quadPose.position.asMeters();
            osg::Vec3 normal = quadPose.orientation * osg::Vec3(0.f, -1.f, 0.f);
            ...
            const float distance = static_cast<float>(((center - origin) * normal) / denom);
            if (distance <= 0.f || distance >= bestDistance)
                continue;

            const osg::Vec3 hit = origin + direction * distance;
            const osg::Vec3 local = quadPose.orientation.inverse() * (hit - center);

            osg::Vec2 extent = layer->mConfig->extent;
            if (layer->mConfig->autoSize)
            {
                layer->updateRect();
                const float width = static_cast<float>(layer->mRect.right - layer->mRect.left);
                const float height = static_cast<float>(layer->mRect.bottom - layer->mRect.top);
                if (width > 0.f && height > 0.f && layer->mConfig->spatialResolution > 0)
                    extent.set(width / layer->mConfig->spatialResolution, height / layer->mConfig->spatialResolution);
            }

            const float width = std::max(0.001f, extent.x());
            const float height = std::max(0.001f, extent.y());
            const float u = static_cast<float>(local.x() / width + 0.5f);
            const float v = static_cast<float>(local.z() / height + 0.5f);
        ...
        setFocusLayer(bestLayer);
        mAndroidOpenXRQuadFocusLayer = bestLayer;
        mAndroidOpenXRQuadFocusUntil = std::chrono::steady_clock::now() + std::chrono::milliseconds(350);

        osg::Vec3 pseudoHitPoint(0.f, 0.f, 0.f);
        pseudoHitPoint.x() = bestLayer->mConfig->center.x() - 0.5f + bestU;
        pseudoHitPoint.z() = bestLayer->mConfig->center.y() - 0.5f + bestV;
        computeGuiCursor(pseudoHitPoint);

        if (distanceMeters)
            *distanceMeters = bestDistance;

        static int sLoggedAndroidQuadPointer = 0;
        if (sLoggedAndroidQuadPointer < 80)
        {
            ++sLoggedAndroidQuadPointer;
            Log(Debug::Info) << "FNV/ESM4 proof: Android VR wrist GUI pointer hit layer=" << bestLayer->mLayerName
                             << " distanceMeters=" << bestDistance << " uv=(" << bestU << "," << bestV << ")"
                             << " local=(" << bestLocal.x() << "," << bestLocal.y() << "," << bestLocal.z() << ")"
                             << " cursor=(" << mGuiCursor.x() << "," << mGuiCursor.y() << ")"
                             << " space=" << (bestLayer->mConfig ? bestLayer->mConfig->space : std::string("<none>"));
        }
        return true;
#else
        (void)pointerPose;
    ...
    bool VRGUIManager::hasAndroidOpenXRQuadFocusLatch() const
    {
#if defined(__ANDROID__)
        return mAndroidOpenXRQuadFocusLayer && mAndroidOpenXRQuadFocusLayer->visible()
            && mAndroidOpenXRQuadFocusLayer->mConfig && std::chrono::steady_clock::now() <= mAndroidOpenXRQuadFocusUntil;
#else
        return false;
#endif
    }
    ...
    void VRGUIManager::computeGuiCursor(osg::Vec3 hitPoint)
    {
        ...
        if (mFocusLayer
            && (mFocusLayer->mLayerName == "VirtualKeyboard"
                || isFalloutInventoryPanelLayer(mFocusLayer->mLayerName) || isFalloutHudLayer(mFocusLayer->mLayerName)))
        {
