...
namespace MWVR
{
    namespace
    {
        ...
        bool shouldLogPointerRay()
        {
            static const bool enabled = getPointerDebugEnvFloat("OPENMW_FNV_VR_POINTER_RAY_LOG", 0.f) != 0.f;
            return enabled;
        }
    ...
    public:
        ...
        MWWorld::Ptr dropItemImpl(const MWGui::ItemStack& item, size_t count, bool copy)
        {
            ...
            MWWorld::Ptr dropped = world->dropObjectOnGround(world->getPlayerPtr(), item.mBase, count);
    ...
    void UserPointer::activate()
    {
        ...
        {
            Log(Debug::Info) << "FNV/ESM4 proof: VR pointer activate ignored while GUI focus is active";
            return;
        }
        ...
        if (mPointerRay.mHit)
        {
            ...
            MWWorld::Ptr actorAssistTarget = findActorAssistTarget(target);
            if (!actorAssistTarget.isEmpty())
            {
                Log(Debug::Info) << "FNV/ESM4 proof: VR pointer actor assist replacing id="
                                 << (target.isEmpty() ? ESM::RefId() : target.getCellRef().getRefId())
                                 << " with id=" << actorAssistTarget.getCellRef().getRefId()
                                 << " name=" << actorAssistTarget.getClass().getName(actorAssistTarget);
                target = actorAssistTarget;
            }

            auto wm = MWBase::Environment::get().getWindowManager();
            if (!target.isEmpty())
            {
                Log(Debug::Info) << "FNV/ESM4 proof: VR pointer activate hit target id="
                                 << target.getCellRef().getRefId() << " type=" << target.getType()
                                 << " name=" << target.getClass().getName(target)
                                 << " distance=" << mDistanceToPointerTarget << " guiMode=" << wm->isGuiMode();
            }
            else
            {
                Log(Debug::Info) << "FNV/ESM4 proof: VR pointer activate hit empty target distance="
                                 << mDistanceToPointerTarget << " guiMode=" << wm->isGuiMode();
            }
            ...
            if (!target.isEmpty())
            {
                ...
                else
                {
                    ...
                    if (!tryProbePick(target))
                    {
                        ...
                        Log(Debug::Info) << "FNV/ESM4 proof: VR pointer deferred activation to Lua handlers id="
                                         << target.getCellRef().getRefId() << " type=" << target.getType()
                                         << " name=" << target.getClass().getName(target);
                    }
                }
            ...
            Log(Debug::Info) << "FNV/ESM4 proof: VR pointer activate missed: no ray hit";
    }

    void UserPointer::hide()
    {
        mCrosshair->hide();
        mPointerRay = {};
    ...
    MWWorld::Ptr UserPointer::findActorAssistTarget(const MWWorld::Ptr& currentTarget) const
    {
        if (!currentTarget.isEmpty() && currentTarget.getClass().isActor())
            return {};
        ...
        const osg::Vec3f origin = mLastPose.position.asMWUnits();
        osg::Vec3f forward = mLastPose.orientation * osg::Vec3f(0.f, 1.f, 0.f);
        if (forward.length2() <= 0.0001f)
            return {};
        forward.normalize();

        const float maxDistance = std::max(world->getMaxActivationDistance() * 1.5f, mDistanceToPointerTarget + 64.f);
        const float maxLateral = 95.f;
        ...
        cell->forEach([&](const MWWorld::Ptr& ptr) {
            ...
            const osg::Vec3f base = ptr.getRefData().getPosition().asVec3();
            const std::array<osg::Vec3f, 3> targetPoints = {
                base + osg::Vec3f(0.f, 0.f, 45.f),
                base + osg::Vec3f(0.f, 0.f, 80.f),
                base + osg::Vec3f(0.f, 0.f, 115.f),
            };

            for (const osg::Vec3f& point : targetPoints)
            {
                osg::Vec3f toActor = point - origin;
                const float dist = toActor.length();
                if (dist <= 0.f || dist > maxDistance)
                    continue;

                toActor /= dist;
                const float dot = forward * toActor;
                if (dot < 0.78f)
                    continue;

                const float lateral = std::sqrt(std::max(0.f, 1.f - dot * dot)) * dist;
                if (lateral > maxLateral)
                    continue;

                const float score = lateral + dist * 0.08f;
                if (score < bestScore)
                {
                    bestScore = score;
                    best = ptr;
        ...
        if (!best.isEmpty())
            Log(Debug::Info) << "FNV/ESM4 proof: VR pointer actor assist candidate id="
                             << best.getCellRef().getRefId() << " name=" << best.getClass().getName(best)
                             << " score=" << bestScore;
    ...
    void UserPointer::update()
    {
        ...
        mDistanceToPointerTarget = Util::getPoseTarget(
            mPointerRay, pose, true,
            MWRender::Mask_3DGUI_NonIntersectable | MWRender::Mask_FirstPerson | MWRender::Mask_Player);
        ...
        if (mPointerRay.mHit && mDistanceToPointerTarget > 0.f)
        {
            ...
            if (shouldLogPointerRay())
            {
                Log(Debug::Info) << "FNV/ESM4 proof: VR pointer ray hit distance=" << mDistanceToPointerTarget
                                 << " object="
                                 << (mPointerRay.mHitObject.isEmpty()
                                             ? ESM::RefId()
                                             : mPointerRay.mHitObject.getCellRef().getRefId())
                                 << " node="
        ...
        else
        {
            ...
            if (shouldLogPointerRay())
            {
                Log(Debug::Info) << "FNV/ESM4 proof: VR pointer ray miss distance=" << mDistanceToPointerTarget;
            }
        }
    }
