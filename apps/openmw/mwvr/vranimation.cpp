...
namespace MWVR
{
    int getActivePlayerWeaponType()
    {
        ...
        MWMechanics::getActiveWeapon(MWBase::Environment::get().getWorld()->getPlayerPtr(), &weaponType);
        return weaponType;
    }
    ...
    void FingerController::operator()(osg::Node* node, osg::NodeVisitor* nv)
    {
        ...
        else if (curlEnabled)
        {
            ...
            if (directPoseOnly)
            {
                ...
                matrix = usedChainPose ? chainProofMatrix : mBaseMatrix;
            }
    ...
    void HandController::operator()(osg::Node* node, osg::NodeVisitor* nv)
    {
        ...
        auto weaponType = getActivePlayerWeaponType();
    ...
    class SetDrawableMaskVisitor : public osg::NodeVisitor
    {
    public:
        SetDrawableMaskVisitor(unsigned int mask)
            : osg::NodeVisitor(TRAVERSE_ALL_CHILDREN)
            , mMask(mask)
        {
        }

        void apply(osg::Drawable& node) override { node.setNodeMask(mMask); }
        void apply(osg::Geometry& node) override { node.setNodeMask(mMask); }
    ...
    public:
        ...
        osg::ref_ptr<osg::Geometry> makeStaticGeometry(osg::Drawable& drawable)
        {
            ...
            const bool enableDynamicStaticHandFingers
                = getEnvFloat("OPENMW_FNV_ENABLE_DYNAMIC_HAND_FINGERS",
#if defined(ANDROID)
                      1.f
#else
                      0.f
            ...
            if (staticHand && (leftHand || rightHand) && hasFingerWeights && enableDynamicStaticHandFingers)
            {
            ...
            else if (staticHand && (leftHand || rightHand) && hasFingerWeights)
            {
                Log(Debug::Warning) << "FNV/ESM4 diag: VR static hand finger deform disabled for stable Android draw"
                                    << " hand=" << (leftHand ? "left" : "right") << " kind=" << kind
    ...
    public:
        ...
        bool mWireframe = getEnvFloat("OPENMW_FNV_VR_HAND_WIREFRAME",
#if defined(ANDROID)
                              0.f
#else
                              0.f
        ...
        bool mWeightDebug = getEnvFloat("OPENMW_FNV_VR_HAND_WEIGHT_DEBUG",
#if defined(ANDROID)
                              0.f
#else
                              0.f
    ...
    public:
        TrackingController(std::shared_ptr<VR::Space> space, std::string auditSourceSpaceName,
            std::shared_ptr<VR::Space> orientationSpace, std::string auditOrientationSpaceName,
            std::shared_ptr<VR::Space> auditAimSpace, std::string auditAimSpaceName, osg::Vec3 baseOffset, bool left,
            bool mirror, bool useNativeGripOrientation, std::string debugName)
            : mSpace(space)
            , mAuditSourceSpaceName(std::move(auditSourceSpaceName))
            , mOrientationSpace(orientationSpace)
            , mAuditOrientationSpaceName(std::move(auditOrientationSpaceName))
        ...
        void update(osg::MatrixTransform& transform)
        {
            ...
            if (!tp.status)
            {
                if (mPoseFailureLogCount < 40 || (++mPoseFailureLogFrame % 300) == 0)
                {
                    ++mPoseFailureLogCount;
                    Log(Debug::Warning) << "FNV/ESM4 diag: VR hand pose locate failed hand=" << mDebugName
                                        << " space=" << mAuditSourceSpaceName;
                }
                return;
            }

            auto orientationTp = tp;
            if (mOrientationSpace != nullptr)
            {
                const auto locatedOrientationTp = mOrientationSpace->locateInWorld();
                if (!!locatedOrientationTp.status)
                    orientationTp = locatedOrientationTp;
                else if (mOrientationFailureLogCount < 20 || (++mOrientationFailureLogFrame % 300) == 0)
                {
                    ++mOrientationFailureLogCount;
                    Log(Debug::Warning) << "FNV/ESM4 diag: VR hand orientation pose locate failed hand=" << mDebugName
                                        << " space=" << mAuditOrientationSpaceName
                                        << "; using position space orientation " << mAuditSourceSpaceName;
                }
            }

            auto orientation = mBaseOrientation * orientationTp.pose.orientation;
            ...
            if (mLeft)
                offset.x() = -offset.x();
            position += orientationTp.pose.orientation * offset;
            ...
            const bool handTrackingLog = getEnvFloat("OPENMW_FNV_VR_HAND_TRACKING_LOG",
                                             0.f
                                             )
                != 0.f;
                ...
                && (mAimAuditLogCount < 40 || (++mAimAuditLogFrame % 300) == 0))
            {
                ...
                if (!!aimTp.status)
                {
                    ...
                    const osg::Vec3 orientationSourceWorld = orientationTp.pose.position.asMWUnits();
                    const osg::Quat orientationSourceOrientation = orientationTp.pose.orientation;
                    ...
                    const osg::Vec3 orientationSourceMinusAimWorld = orientationSourceWorld - aimWorld;
                    const osg::Vec3 orientationSourceMinusAimLocal
                        = aimOrientation.inverse() * orientationSourceMinusAimWorld;
                    ...
                    const osg::Quat orientationSourceFromAim = orientationSourceOrientation * aimOrientation.inverse();
                    ...
                    double orientationSourceAngle = 0.0;
                    osg::Vec3 orientationSourceAxis;
                    orientationSourceFromAim.getRotate(orientationSourceAngle, orientationSourceAxis);
                    ...
                    Log(Debug::Info) << "FNV/ESM4 diag: VR hand aim-frame audit hand=" << mDebugName
                                     ...
                                     << " orientationSpace=" << mAuditOrientationSpaceName
                                     ...
                                     << " orientationSourceMinusAimWorld=(" << orientationSourceMinusAimWorld.x()
                                     << "," << orientationSourceMinusAimWorld.y() << ","
                                     << orientationSourceMinusAimWorld.z() << ")"
                                     << " orientationSourceMinusAimLocal=(" << orientationSourceMinusAimLocal.x()
                                     << "," << orientationSourceMinusAimLocal.y() << ","
                                     << orientationSourceMinusAimLocal.z() << ")"
                                     << " orientationSourceDistance=" << orientationSourceMinusAimWorld.length()
                                     ...
                                     << "," << sourceAxis.z() << ")"
                                     << " orientationSourceRotationDeltaDeg="
                                     << osg::RadiansToDegrees(orientationSourceAngle)
                                     << " orientationSourceRotationAxis=(" << orientationSourceAxis.x() << ","
                                     << orientationSourceAxis.y() << "," << orientationSourceAxis.z() << ")"
            ...
            {
                std::string sourceViewText = " sourceViewMW=(unavailable)";
                if (auto viewSpace = VR::Session::instance().getReferenceSpace(VR::ReferenceSpace::View))
                {
                    const auto viewTp = mSpace->locate(*viewSpace);
                    if (!!viewTp.status)
                    {
                        const osg::Vec3 sourceView = viewTp.pose.position.asMWUnits();
                        sourceViewText = " sourceViewMW=(" + std::to_string(sourceView.x()) + ","
                            + std::to_string(sourceView.y()) + "," + std::to_string(sourceView.z()) + ")";
                    }
                }
                ++mAlignmentLogCount;
                Log(Debug::Info) << "FNV/ESM4 diag: VR hand pointer alignment hand=" << mDebugName
                                 ...
                                 << handMatrix.getTrans().z() << ")" << sourceViewText;
            }
        ...
        std::shared_ptr<VR::Space> mOrientationSpace;
        std::string mAuditOrientationSpaceName;
        ...
        int mPoseFailureLogCount = 0;
        int mPoseFailureLogFrame = 0;
        int mOrientationFailureLogCount = 0;
        int mOrientationFailureLogFrame = 0;
        bool mLeft;
        bool mMirror;
    ...
    {
    public:
        FalloutHandControllerSpacePosition(std::string side, std::string model, std::string anchorMode,
            osg::Vec3f modelControllerAnchor, osg::Vec3f controllerLocalTarget, osg::Quat controllerLocalAttitude)
            : mSide(std::move(side))
            , mModel(std::move(model))
            , mAnchorMode(std::move(anchorMode))
            , mModelControllerAnchor(modelControllerAnchor)
                ...
                && node->getNumParents() > 0)
            {
                ...
                if (!parentPaths.empty())
                {
                    ...
                    if (worldToParent.invert(parentLocalToWorld) && worldToParentRotation.invert(parentWorldRotation))
                    {
                        ...
                        const osg::Vec3f rotatedModelAnchor = mModelControllerAnchor * localRotation;
                        const osg::Vec3f localPosition = localTarget - rotatedModelAnchor;
                        transform->setAttitude(localRotation.getRotate());
                        transform->setPosition(localPosition);
                        ...
                        if (mLogCount < 20 || (++mLogFrame % 300) == 0)
                        {
                            ...
                            Log(Debug::Info) << "FNV/ESM4 diag: VR hand controller-space position solve side="
                                             ...
                                             << " modelControllerAnchor=(" << mModelControllerAnchor.x() << ","
                                             << mModelControllerAnchor.y() << "," << mModelControllerAnchor.z() << ")"
                                             << " targetWorld=(" << targetWorld.x() << "," << targetWorld.y() << ","
                                             << targetWorld.z() << ")"
                                             << " localTarget=(" << localTarget.x() << "," << localTarget.y() << ","
                                             << localTarget.z() << ")"
                                             << " rotatedModelAnchor=(" << rotatedModelAnchor.x() << ","
                                             << rotatedModelAnchor.y() << "," << rotatedModelAnchor.z() << ")"
    ...
    private:
        std::string mSide;
        std::string mModel;
        std::string mAnchorMode;
        osg::Vec3f mModelControllerAnchor;
        ...
        , mSceneRoot(sceneRoot)
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            const std::string requestedSpace = getEnvString("OPENMW_FNV_VR_HAND_TRACKING_SPACE", "wrist");
            const bool wristAvailable = xrInput.getSpace(wristSpace) != nullptr;
            const bool useAim = requestedSpace == "aim";
            const bool useGrip = requestedSpace == "grip";
            const bool useWristTop = !useAim && !useGrip && wristAvailable;
            if (useAim)
                ctx.spaceName = aimSpace;
            else if (useWristTop)
                ctx.spaceName = wristSpace;
            else if (requestedSpace == "wrist" && !wristAvailable)
                Log(Debug::Warning) << "FNV/ESM4 diag: VR wrist hand space missing " << wristSpace
                                    << "; using grip tracking space " << gripSpace;
            const osg::Vec3 defaultGripOffset = i == 0
                ? osg::Vec3(-2.690756f, 10.312318f, -5.714692f)
                : osg::Vec3(0.077705f, 10.069760f, -4.447718f);
            const osg::Vec3 offset = useWristTop ? osg::Vec3(0, 0, 0)
                                                 : osg::Vec3(
                                                       getHandEnvFloat(i == 0, "GRIP_OFFSET_X",
                                                           getEnvFloat("OPENMW_FNV_VR_HAND_GRIP_OFFSET_X",
                                                               defaultGripOffset.x())),
                                                       getHandEnvFloat(i == 0, "GRIP_OFFSET_Y",
                                                           getEnvFloat("OPENMW_FNV_VR_HAND_GRIP_OFFSET_Y",
                                                               defaultGripOffset.y())),
                                                       getHandEnvFloat(i == 0, "GRIP_OFFSET_Z",
                                                           getEnvFloat("OPENMW_FNV_VR_HAND_GRIP_OFFSET_Z",
                                                               defaultGripOffset.z())));
            const bool useNativeGripOrientation = useWristTop;
            const bool handSourceLog = getEnvFloat("OPENMW_FNV_VR_HAND_SOURCE_LOG", 0.f) != 0.f;
            if (handSourceLog)
                Log(Debug::Info) << "FNV/ESM4 diag: VR hand tracking source hand=" << (i == 0 ? "left" : "right")
                                 << " space=" << ctx.spaceName
                                 << " requested=" << requestedSpace
                                 << " nativeOrientation=" << useNativeGripOrientation
                                 << " baseOffset=(" << offset.x() << "," << offset.y() << "," << offset.z() << ")";
            const std::string requestedOrientationSpace = getEnvString(
                "OPENMW_FNV_VR_HAND_ORIENTATION_SPACE", useWristTop ? "source" : "aim");
            std::string orientationSpaceName = ctx.spaceName;
            if (requestedOrientationSpace == "aim")
                orientationSpaceName = aimSpace;
            else if (requestedOrientationSpace == "grip")
                orientationSpaceName = gripSpace;
            else if (requestedOrientationSpace == "wrist" && wristAvailable)
                orientationSpaceName = wristSpace;
            const bool useNativeControllerOrientation = useWristTop || orientationSpaceName == aimSpace;
            if (handSourceLog)
                Log(Debug::Info) << "FNV/ESM4 diag: VR hand orientation source hand=" << (i == 0 ? "left" : "right")
                                 << " space=" << orientationSpaceName
                                 << " requested=" << requestedOrientationSpace
                                 << " nativeOrientation=" << useNativeControllerOrientation;
#else
            if (VR::getLeftHandedMode())
                ctx.spaceName = i == 1 ? OpenXRInput::LeftHandGrip : OpenXRInput::RightHandGrip;
            else
            ...
            const bool useNativeGripOrientation = useWristTop;
            if (getEnvFloat("OPENMW_FNV_VR_HAND_SOURCE_LOG", 0.f) != 0.f)
                Log(Debug::Info) << "FNV/ESM4 diag: VR hand tracking source hand=" << (i == 0 ? "left" : "right")
                                 << " space=" << ctx.spaceName
                                 << " nativeOrientation=" << useNativeGripOrientation
                                 << " baseOffset=(" << offset.x() << "," << offset.y() << "," << offset.z() << ")";
            const std::string orientationSpaceName = ctx.spaceName;
            const bool useNativeControllerOrientation = useNativeGripOrientation;
#endif
            ctx.forearmBone = i == 0 ? "Bip01 L Forearm" : "Bip01 R Forearm";
            ctx.forearmController = std::make_unique<TrackingController>(
                xrInput.getSpace(ctx.spaceName), ctx.spaceName, xrInput.getSpace(orientationSpaceName),
                orientationSpaceName, xrInput.getSpace(aimSpace), aimSpace, offset, i == 0, VR::getLeftHandedMode(),
                useNativeControllerOrientation, i == 0 ? "left" : "right");
...
#if defined(ANDROID)
        mFalloutVrHandSurfaces = {
            { "characters/_male/lefthandpipboyglove.nif", {}, "default-player-left-pipboy-glove", true },
            { "pipboy3000/pipboyarm.nif", {}, "default-player-left-pipboy-arm", true },
            { "characters/_male/righthand.nif", {}, "default-player-right-hand", false },
            { "pipboy3000/pipboyarm.nif", {}, "right-pipboy-calibration", false },
        };
#else
        mFalloutVrHandSurfaces = {
            { "characters/_male/lefthand.nif", {}, "default-player-left-hand", true },
            { "characters/_male/righthand.nif", {}, "default-player-right-hand", false },
        };
#endif

        Log(Debug::Warning) << "FNV/ESM4 diag: seeded default FNV VR hand surfaces count="
                            << mFalloutVrHandSurfaces.size();
        Log(Debug::Warning) << "FNV/ESM4 diag: forcing player VRAnimation into VM_VRFirstPerson at construction";
        setViewMode(VM_VRFirstPerson);
    }
    ...
    void VRAnimation::updateParts()
    {
        ...
        static int sLoggedUpdateParts = 0;
        if (sLoggedUpdateParts < 24)
        {
            ++sLoggedUpdateParts;
            Log(Debug::Warning) << "FNV/ESM4 diag: VRAnimation updateParts viewMode="
                                << static_cast<int>(mViewMode)
                                << " handSurfaces=" << mFalloutVrHandSurfaces.size();
        }
...
#if defined(ANDROID)
        SetDrawableMaskVisitor hidePlayerDrawableVisitor(0);
        mObjectRoot->accept(hidePlayerDrawableVisitor);
#endif
    ...
    void VRAnimation::attachFalloutVrHandSurfaces()
    {
        ...
        for (const FalloutVrHandSurface& surface : mFalloutVrHandSurfaces)
        {
            ...
            if (staticizedRiggedHandPart && directStaticHandNode != nullptr)
            {
                ...
                const float defaultHandRotX = 90.f;
                const float defaultHandRotZ = surface.left ? -25.f : 25.f;
                const float handRotX = getHandEnvFloat(surface.left, "ROT_X", defaultHandRotX);
                const float handRotY = getHandEnvFloat(surface.left, "ROT_Y", surface.left ? -90.f : 90.f);
                const float handRotZ = getHandEnvFloat(surface.left, "ROT_Z", defaultHandRotZ);
            ...
            if (staticizedRiggedHandPart && staticizedHandBounds.valid())
            {
                    ...
                    = dynamic_cast<osg::PositionAttitudeTransform*>(attached.get()))
                {
                    ...
                    const std::size_t socketIndex = surface.left ? 0 : 1;
                    const float defaultAnchorPipBoy = 1.f;
                    const bool anchorToPipBoy = pipBoySocketTargetValid[socketIndex]
                        && getHandEnvFloat(surface.left, "ANCHOR_PIPBOY", defaultAnchorPipBoy) != 0.f;
                    if (anchorToPipBoy)
                    {
                        targetCuffAnchor = pipBoySocketTargets[socketIndex];
                        const osg::Vec3f defaultSocketOffset(1.5f, -2.f, -2.f);
                        targetCuffAnchor += osg::Vec3f(getHandEnvFloat(surface.left, "SOCKET_X", defaultSocketOffset.x()),
                            getHandEnvFloat(surface.left, "SOCKET_Y", defaultSocketOffset.y()),
                            getHandEnvFloat(surface.left, "SOCKET_Z", defaultSocketOffset.z()));
                    }
                    else
                    {
                        const osg::Vec3f defaultHandOffset(0.f, surface.left ? 0.f : 12.f, surface.left ? 0.f : 4.f);
                        targetCuffAnchor += osg::Vec3f(getHandEnvFloat(surface.left, "OFFSET_X", 0.f),
                            getHandEnvFloat(surface.left, "OFFSET_Y", defaultHandOffset.y()),
                    ...
                    if (frameSolve)
                    {
                        ...
                        const float defaultSolveRoll = -5.f;
                        const float solveRoll = getHandEnvFloat(surface.left, "SOLVE_ROLL", defaultSolveRoll);
                        if (solveRoll != 0.f)
                    ...
                    const bool useControllerSpacePosition = getHandEnvFloat(surface.left, "CONTROLLER_SPACE_POSITION",
                        getEnvFloat("OPENMW_FNV_VR_HAND_CONTROLLER_SPACE_POSITION", 0.f)) != 0.f;
#else
                    const bool useControllerSpacePosition
                        = !surface.left && getHandEnvFloat(surface.left, "CONTROLLER_SPACE_POSITION", 0.f) != 0.f;
#endif
                    if (useControllerSpacePosition)
                    {
                        const std::string controllerAnchorMode
                            = getEnvString("OPENMW_FNV_VR_HAND_CONTROLLER_ANCHOR", "cuff");
                        osg::Vec3f modelControllerAnchor = modelCuffAnchor;
#if defined(ANDROID)
                        if (controllerAnchorMode == "center" || controllerAnchorMode == "palm")
                        {
                            modelControllerAnchor = osg::Vec3f(
                                scale.x() * getHandEnvFloat(surface.left, "CONTROLLER_ANCHOR_X", center.x()),
                                scale.y() * getHandEnvFloat(surface.left, "CONTROLLER_ANCHOR_Y", center.y()),
                                scale.z() * getHandEnvFloat(surface.left, "CONTROLLER_ANCHOR_Z", center.z()));
                        }
                        else if (controllerAnchorMode == "cuff")
                        {
                            modelControllerAnchor = modelCuffAnchor;
                        }
#endif
                        osg::Vec3f controllerLocalTarget = targetCuffAnchor;
#if defined(ANDROID)
                        controllerLocalTarget = osg::Vec3f(
                            getHandEnvFloat(surface.left, "CONTROLLER_TARGET_X",
                                getEnvFloat("OPENMW_FNV_VR_HAND_CONTROLLER_TARGET_X", targetCuffAnchor.x())),
                            getHandEnvFloat(surface.left, "CONTROLLER_TARGET_Y",
                                getEnvFloat("OPENMW_FNV_VR_HAND_CONTROLLER_TARGET_Y", targetCuffAnchor.y())),
                            getHandEnvFloat(surface.left, "CONTROLLER_TARGET_Z",
                                getEnvFloat("OPENMW_FNV_VR_HAND_CONTROLLER_TARGET_Z", targetCuffAnchor.z())));
#endif
                        transform->addUpdateCallback(new FalloutHandControllerSpacePosition(
                            auditSide, correctedModel.value(), controllerAnchorMode, modelControllerAnchor, controllerLocalTarget,
            ...
            if (pipBoyArm)
            {
                    ...
                    = dynamic_cast<osg::PositionAttitudeTransform*>(attached.get()))
                {
                    ...
                    if (rightPipBoyCalibration)
                    {
                        pipBoyVisualOffset = osg::Vec3f(getPipBoyEnvFloat(false, "VISUAL_OFFSET_X", 3.f),
                            getPipBoyEnvFloat(false, "VISUAL_OFFSET_Y", 0.f),
                            getPipBoyEnvFloat(false, "VISUAL_OFFSET_Z", 0.25f));
                    ...
                    if (rightPipBoyCalibration)
                    {
                        if (getEnvFloat("OPENMW_FNV_RIGHT_PIPBOY_DEBUG_STYLE", 1.f) != 0.f)
    ...
    void VRAnimation::updateLocalSpaceWorldPose()
    {
        ...
        Stereo::Pose referencePose = originPose + mCharLocalSpacePose;
#if defined(ANDROID)
        auto position = referencePose.position.asMWUnits();
        position += osg::Vec3f(-1.45f, 3.15f, -6.2f);
        referencePose.position = Stereo::Position::fromMWUnits(position);

        static int sAndroidVrOriginLogs = 0;
        if (sAndroidVrOriginLogs < 16)
        {
            ++sAndroidVrOriginLogs;
            Log(Debug::Warning) << "Android VR origin calibration: shared reference poseMW=(" << position.x() << ","
                                << position.y() << "," << position.z() << ")";
        }
#endif
        XR::Session::instance().setReferenceWorldPose(referencePose);
    }
    ...
    {
        static int sLoggedProfileActive = 0;
        if (sLoggedProfileActive < 16)
        {
            ++sLoggedProfileActive;
            Log(Debug::Info) << "FNV/ESM4 diag: VR interaction profile active changed without rebuilding hand parts"
                             << " path=" << topLevelPath << " active=" << isActive;
        }
