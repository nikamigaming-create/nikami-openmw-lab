...
namespace
{
    ...
    bool isFalloutVrPlayerBodyAnimationGroup(std::string_view groupname)
    {
        return groupname == "idle" || groupname.starts_with("idle") || groupname.starts_with("walk")
            || groupname.starts_with("run") || groupname.starts_with("sneak") || groupname.starts_with("swim")
...
namespace MWRender
{
        ...
        std::string_view mode, osg::Quat rotation, osg::Quat bindRotation, const std::string& lowerBone)
    {
        ...
        if (mode == "rawUpperXThighsYCalves")
        {
            if (isFalloutThighBone(lowerBone))
                return falloutHalfTurn('x') * rotation * bindRotation;
            if (isFalloutCalfFootBone(lowerBone))
                return falloutHalfTurn('y') * rotation * bindRotation;
            return rotation;
        ...
        const std::string& kfname, const std::string& baseModel, bool falloutProcedureIdle)
    {
        ...
        if (useSmoothAnimationTransitions())
        {
            ...
            blendConfigPath.changeExtension(VFS::Path::ExtensionView("yaml"));
        ...
        bool loopfallback)
    {
        ...
        if (suppressFalloutVrPlayerBodyAnimations(mPtr) && isFalloutVrPlayerBodyAnimationGroup(groupname))
        {
            static unsigned int sSuppressedVrPlayerBodyLogs = 0;
            if (mStates.erase(std::string(groupname)) > 0)
                resetActiveGroups();
            if (sSuppressedVrPlayerBodyLogs < 32)
            {
                ++sSuppressedVrPlayerBodyLogs;
                Log(Debug::Info) << "FNV/ESM4 diag: suppressed VR player render animation group '" << groupname
        ...
        for (; iter != mAnimSources.rend(); ++iter)
        {
            ...
            if (reset(state, textkeys, groupname, start, stop, startpoint, loopfallback))
            {
                ...
                if (isFalloutNpc(mPtr) && isFalloutSeededIdleGroup(groupname) && startpoint == 0.f
...
//## VR_PATCH END
    ...
    {
        return Settings::game().mSmoothAnimTransitions;
    }
