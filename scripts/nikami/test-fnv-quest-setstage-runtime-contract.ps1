...
$ProofDir = Join-Path $ProofRoot "fnv-quest-setstage-runtime-contract/$Stamp"
...
Write-ProofLine "FNV quest SetStage/GetStage runtime contract $Stamp"
...
Write-ProofLine ""
Write-ProofLine "Runtime boundary: This proves selected Fallout-native SetStage/GetStage MWScript opcodes are compiler-bound and runtime-bound on PC-flat. SetStage adds the real harvested QUST stage, executes its selected stage result fragment, and GetStage reads the resulting stage. It does not claim exhaustive condition evaluation, target marker routing, full result opcode coverage, rewards, or full quest completion parity."
Write-ProofLine ""

Assert-Text "components/compiler/opcodes.hpp" "opcodeSetStage = 0x200032b" "SetStage opcode id"
Assert-Text "components/compiler/opcodes.hpp" "opcodeGetStage = 0x200032c" "GetStage opcode id"
Assert-Text "components/compiler/extensions0.cpp" 'extensions.registerInstruction("setstage", "cl", opcodeSetStage)' "SetStage compiler instruction"
Assert-Text "components/compiler/extensions0.cpp" 'extensions.registerFunction("getstage", ''l'', "c", opcodeGetStage)' "GetStage compiler function"
Assert-Text "apps/openmw/mwscript/dialogueextensions.cpp" "setQuestStageFromScript" "shared script stage mutation path"
Assert-Text "apps/openmw/mwscript/dialogueextensions.cpp" "class OpSetStage" "SetStage runtime opcode"
Assert-Text "apps/openmw/mwscript/dialogueextensions.cpp" "class OpGetStage" "GetStage runtime opcode"
Assert-Text "apps/openmw/mwscript/dialogueextensions.cpp" "installSegment5<OpSetStage>" "SetStage opcode installed"
Assert-Text "apps/openmw/mwscript/dialogueextensions.cpp" "installSegment5<OpGetStage>" "GetStage opcode installed"
Assert-Text "apps/openmw/mwscript/docs/vmformat.txt" "op 0x200032b: SetStage" "VM docs SetStage"
Assert-Text "apps/openmw/mwscript/docs/vmformat.txt" "op 0x200032c: GetStage" "VM docs GetStage"
Assert-Text "apps/openmw/mwdialogue/journalimp.cpp" "executeScript(info->mResultScript" "stage result fragment executes through journal add-entry"
Assert-Text "scripts/nikami/fnv_no_silent_skip_classification.py" '"QUST": "quest records are stored, QUST stage journal entries are bridged, selected SetStage/GetStage/GetStageDone, StartQuest/CompleteQuest/GetQuestCompleted/GetQuestRunning, and stage fragments execute, selected objective target references can resolve, and objective displayed/completed script state is bound pending full quest lifecycle/HUD-marker/condition parity"' "QUST remains loaded-pending runtime with selected SetStage/GetStage support"
...
$SetStageStartupScript = Join-Path $ProofDir "quest-setstage-opcodes-startup.txt"
@(
    "SetStage $QuestId $StageIndex",
    "GetStage $QuestId",
    "GetObjectiveDisplayed $QuestId $ObjectiveIndex"
) | Set-Content -LiteralPath $SetStageStartupScript -Encoding ASCII
Write-ProofLine "SetStage opcode startup script: $SetStageStartupScript"
...
& $FlatProofScript `
    ...
    -StartupScript $SetStageStartupScript `
...
$runtimeMatch = Assert-FileContains $OpenMwLog "FNV/ESM4 proof: quest journal MWScript opcode SetStage .*quest=.*$QuestId" "runtime SetStage proof"
Assert-FileContains $OpenMwLog "FNV/ESM4 proof: quest journal MWScript opcode GetStage .*quest=.*$QuestId.*currentIndex=$StageIndex" "runtime GetStage proof" | Out-Null
Assert-FileContains $FlatSummary "^FnvQuestJournalScriptTrace: True$" "flat proof required journal trace" | Out-Null
...
$metadata = [ordered]@{
    ...
    classifications = @(
        [ordered]@{
            system = "selected FNV SetStage/GetStage quest stage execution"
            classification = "runtime-supported"
            proof = "SetStage VMS55 10 executes the selected harvested stage fragment, GetStage reads stage 10, and objective 5 displayed is set through MWScript."
        },
        [ordered]@{
            system = "conditions, HUD markers, target routing, rewards, full quest completion"
            classification = "loaded-pending-runtime"
            proof = "Selected SetStage/GetStage and fragment execution work, but full quest parity remains separate gates."
        }
    )
    runtimeBoundary = "Selected FNV SetStage/GetStage quest stage execution is runtime-supported; conditions, HUD markers, target routing, rewards, and full quest completion remain loaded-pending-runtime."
...
$metadataPath = Join-Path $ProofDir "fnv-quest-setstage-runtime-contract.json"
...
Write-ProofLine "FNV quest SetStage/GetStage runtime contract PASS"
Write-ProofLine "ProofDir: $ProofDir"
